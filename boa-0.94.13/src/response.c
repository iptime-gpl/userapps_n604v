...
/* $Id: response.c,v 1.21 2017/03/14 08:35:32 hyrtlac Exp $*/
...
void send_r_not_found(request * req)
{
...
#ifdef USE_ROUTER_MGMT_PORT
        mgmt_port=get_router_mgmt_port();
#else
        mgmt_port=80;
#endif

#if defined(USE_URL_REDIRECT) || defined(USE_NETDETECT)
	char url[256];
	char srv_ip[16];
...
#ifdef USE_IPTIME_SERVICE_NETWORK
	sprintf(srv_ip, "%s.254", srv_net);
#else
	sprintf(srv_ip, "192.168.255.254");
#endif

#ifdef USE_URL_REDIRECT
	if (get_url_redirect_option(url) && !strcmp(req->local_ip_addr, srv_ip))
	{      
		sprintf(url, "http://%s:%d/login/urlredir.cgi?page=url_redirect", srv_ip,mgmt_port);
		send_r_moved_temp(req, url, "");
    		req_flush(req);
		return;
	}
#endif
#ifdef USE_IPTIME_SERVICE_NETWORK
#ifdef USE_NETDETECT
	sprintf(srv_ip, "%s.250", srv_net);
	if (netdetect_get_on_off() && !strcmp(req->local_ip_addr, srv_ip))
	{
        	sprintf(url, "http://%s.1:%d/nd-bin/netdetect.cgi?flag=nd-warning", srv_net, mgmt_port);
        	send_r_moved_temp(req, url, "");
    		req_flush(req);
		return;
	}
#endif
#endif
#endif // defined(USE_URL_REDIRECT) || defined(USE_NETDETECT)

#if defined USE_URL_REDIRECT_FOR_EXTENDER || defined USE_FAKE_DNS
#ifdef USE_URL_REDIRECT_FOR_EXTENDER
	if (!get_upper_ap_status() && get_fakedns())
#else
#ifdef USE_FAKE_DNS
	if(get_fakedns() && (!get_wan_link(WAN1_NAME) || get_wan_ip(WAN1_NAME,wan_ip)))
#endif
#endif
	{      
		char local_ip[32],sm[32],url2[256];

		get_ifconfig(IF_LOCAL,local_ip,sm);
		sprintf(url2, "http://%s:%d",local_ip, mgmt_port);
		send_r_moved_temp(req, url2, "");
    		req_flush(req);
		return;
	}
#endif
...
#if defined USE_SYSINFO && defined USE_WL_IPTIME_HELPER
	{
		...
		if(sw.wl_helper.redirect && wl_helper_is_started())
		{
			char url2[256];

			if(strcmp(sw.wl_helper.url,"")) sprintf(url2, "http://%s",sw.wl_helper.url);
			else if(strcmp(sw.wl_helper.ip,"")) sprintf(url2, "http://%s",sw.wl_helper.ip);
			else 
			{
				char local_ip[32],sm[32];

				get_ifconfig(IF_LOCAL,local_ip,sm);
				sprintf(url2, "http://%s",local_ip);
			}
		...
		if(sw.wl_helper2.redirect && wl_helper_is_started())
		{
			char url2[256];

			if(strcmp(sw.wl_helper2.url,"")) sprintf(url2, "http://%s",sw.wl_helper2.url);
			else if(strcmp(sw.wl_helper2.ip,"")) sprintf(url2, "http://%s",sw.wl_helper2.ip);
			else 
			{
				char local_ip[32],sm[32];
...
#else
#if 1
        req_write(req,"<html><script> if(parent && parent.parent) parent.parent.location = \"/\";</script></html>");
#endif
#endif
    }
    req_flush(req);
