...
void init_iptables(void)
{
	...
	//system("/sbin/iptables -P INPUT ACCEPT");
        ...
        sprintf(cmd, "/sbin/iptables -t nat -D PREROUTING -d 224.0.0.0/4 -j RETURN");
        system(cmd);

        sprintf(cmd, "/sbin/iptables -t nat -I PREROUTING -d 224.0.0.0/4 -j RETURN");
        system(cmd);

	system("/sbin/iptables -D INPUT -d 224.0.0.0/4 -j ACCEPT");
	system("/sbin/iptables -I INPUT -d 224.0.0.0/4 -j ACCEPT");
}
