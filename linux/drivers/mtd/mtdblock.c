 ...
 *
 * $Id: mtdblock.c,v 1.1.1.1 2010/06/15 01:48:54 ysyoo Exp $
 *
...
#if 0  //sc_yang always use one setting 
int flash_hw_start=0x6000;	// hw setting start address
int flash_ds_start=0x6400;	// default setting start address
int flash_ds_len=0xc00;		// default setting length
int flash_cs_start=0x7000;	// current setting start address
int flash_cs_len=0x1000;	// current setting length
...
//---------------------
	...
	while (len > 0) {
		...
		if (size == sect_size) {
			 ...
			 */
// for debug
//printk("do_cached_write: do erase_write: do erase_write, from: 0x%x, len: 0x%x\n", pos, size);
...
// for debug
//printk("do_cached_write: do write_cached_data: from: 0x%x, len: 0x%x\n", mtdblk->cache_offset, mtdblk->cache_size);
...
// for debug
//printk("do_cached_write: MTD_READ, from: 0x%x, len: 0x%x\n", sect_start, sect_size);
...
// for debug
//printk("%s: sector=%d, nr_sectors=%d\n", ((req->cmd == READ) ? "MTD_READ" : "MTD_WRITE"),
