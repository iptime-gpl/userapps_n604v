...
const struct spi_flash_db   spi_flash_known[] =
{
    ...
    {0x1c, 0x31,0},             /*  EON */
#endif
	{0xc8, 0x40,0x16},   		/*  GigaDevice Flash  */ //GD25Q32
	{0xc8, 0x40,0x17},   		/*  GigaDevice Flash  */ //GD25Q32
 ...
 * SPI Flash Info
 */
struct spi_flash_type   spi_flash_info[1];
...
//__TESSPIIRAM 
#if defined(CONFIG_RTL8196C)||defined(CONFIG_RTL8198) ||  defined(CONFIG_RTL_8196E) || defined(CONFIG_RTL_8196C_8196E)
      ...
      *(volatile unsigned int *) SFCSR = LENGTH(0) | CS(3) | READY(1);
      ...
      for (i = 0; i < sizeof(spi_flash_known) / sizeof(struct spi_flash_db); i++)
      {
              ...
              (spi_flash_info[cnt].type_id == spi_flash_known[i].type_id) )
         {
			...
			printk("capacity_id %x\n",spi_flash_info[cnt].capacity_id );
         }


#ifdef CONFIG_EFM_PATCH
            /* scchoi added */
            if(i ==  (sizeof(spi_flash_known) / sizeof(struct spi_flash_db)))
            {
                    printk("Use Default SPI info:\n");
...
#ifdef CONFIG_EFM_PATCH_FOR_4MB
#define CONFIG_MYDLINK_4M_FLASH
#endif
...
// Support for MX2fL series flash 
	...
	},
   {
	...
	},
	{
		...
		dev_id: 0x40,
		name: "winbond",
		#if defined(CONFIG_MYDLINK_4M_FLASH)
...
#ifdef CONFIG_EFM_PATCH
static const struct spi_flash_info default_flash_table = {
                mfr_id: 0xef,
                dev_id: 0x40,
                name: "Unknown(Winbond)",
#ifdef CONFIG_EFM_PATCH_FOR_4MB
                DeviceSize: 0x400000,
#else
                DeviceSize: 0x200000,
...
struct spi_chip_info *spi_probe_flash_chip(struct map_info *map, struct chip_probe *cp)
{
	...
	{
		//spin_lock_irqsave(spi_lock,flags);
		REG32(0xb8000040)=REG32(0xb8000040) &(~0x03FC0000);
	}
#endif
	checkICver();
	spi_pio_init();
	...
	for (i=0; i < (sizeof(flash_tables)/sizeof(struct spi_flash_info)); i++) {
		...
		if ( (spi_flash_info[0].maker_id == spi_flash_known[i].maker_id) &&
			...
			if (chip_info) {
				...
				if (spi_flash_info[0].maker_id == 0xC8){
					printk("\r\nMXIC matched!!");
					chip_info->flash->DeviceSize = 1 << spi_flash_info[0].capacity_id;
				}
...
#ifdef CONFIG_EFM_PATCH
        /* scchoi added */
        if( spi_flash_info[0].maker_id  != 0x0 )
        {
                printk("Use default spi driver info: %02x %02x\n", spi_flash_info[0].maker_id, spi_flash_info[0].type_id );
                chip_info = spi_suzaku_setup(map);
                if (chip_info) {
                        chip_info->flash      = &default_flash_table;
                        ...
                        if (default_flash_table.EraseSize == 4096) //sector or block erase
                        {
                                chip_info->erase      = do_spi_erase;
                                chip_info->write      = do_spi_write;
                        }
