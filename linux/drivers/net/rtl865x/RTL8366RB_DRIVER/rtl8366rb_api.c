...
* $Id: rtl8366rb_api.c,v 1.1.1.1 2010/06/15 01:48:52 ysyoo Exp $
*/
/*	@doc rtl8366rb_api
...
int32 rtl8366rb_getLUTUnicast(rtl8366rb_l2_entry *ptr_l2_entry)
{
	...
	for(i = 0; i < 4; i++)
	{
		...
		else if(_rtl8366rb_cmp(ptr_l2_entry->mac.octet, l2Table.mac.octet, ETHER_ADDR_LEN) == SUCCESS)
		{
            ...
            {
                if(ptr_l2_entry->age == 0) /* Dynamic entry && age == 0 */
...
int32 rtl8366rb_initChip(void)
{
    ...
    const uint32 chipDefault2[][2] = {{0x0000, 0x0830},{0x0400, 0x8130},{0x000A, 0x83ED},{0x0431, 0x5432},
                                   ...
                                   {0xBE10, 0x8140},{0xBE00, 0x1340},{0x0450, 0x0000},{0xFFFF, 0xABCD}};
...
*/
int32 rtl8366rb_setRtctTesting(enum PORTID port)
{
	if(port >= PORT_MAX)
		return ERRNO_INPUT;
...
*/
int32 rtl8366rb_getRtctResult(enum PORTID port,enum CHANNELID channel, uint32* length, uint32* impmis, uint32* openSts, uint32* shortSts)
{
    if(port >= PORT_MAX)
		return ERRNO_INPUT;
