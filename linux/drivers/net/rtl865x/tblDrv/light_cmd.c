...
static int32 _rtl865x_dumpCmds(uint32 userId,  int32 argc,int8 **saved)
{
	 ...
	 else if ( strcmp(nextToken, "netif") == 0 )
	{
	 ...
	 else if ( strcmp(nextToken, "acl") == 0 )
	 {
		...
		uint32 entry;
		int multicast_tbl_size;
		
#if defined(CONFIG_RTL_8196C_8196E)		
		if(IS_RTL8196E)
			multicast_tbl_size = RTL8196E_MULTICASTTBL_SIZE;
		else
			multicast_tbl_size = RTL8196C_MULTICASTTBL_SIZE;
#else
		multicast_tbl_size = RTL8651_MULTICASTTBL_SIZE;
#endif
		
		rtlglue_printf(">>ASIC Multicast Table:\n");
		for(entry=0; entry<multicast_tbl_size; entry++) {
