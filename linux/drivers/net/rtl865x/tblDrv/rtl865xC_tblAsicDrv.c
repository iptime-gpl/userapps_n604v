...
static int rtl8651_ipMulticastTableIndex_96e(ipaddr_t srcAddr, ipaddr_t dstAddr)
{
	uint32 idx;
...
static int rtl8651_ipMulticastTableIndex_96c(ipaddr_t srcAddr, ipaddr_t dstAddr) 
{
	uint32 idx;
	uint32 sip[32],dip[32];
	uint32 hash[7];
...
uint32 rtl8651_ipMulticastTableIndex(ipaddr_t srcAddr, ipaddr_t dstAddr) 
{
	...
	if(IS_RTL8196E)
		return rtl8651_ipMulticastTableIndex_96e(srcAddr, dstAddr);
	else
		return rtl8651_ipMulticastTableIndex_96c(srcAddr, dstAddr);
#elif defined(CONFIG_RTL_8196E)
	return rtl8651_ipMulticastTableIndex_96e(srcAddr, dstAddr);
	
#elif defined(CONFIG_RTL8196C_REVISION_B) || defined(CONFIG_RTL8198) || defined(CONFIG_RTL_8196D)
	return rtl8651_ipMulticastTableIndex_96c(srcAddr, dstAddr);
...
int update_ip_multicast_tbl(uint32  index, uint8 mem_port, int valid)
{
	...
	if(index < ip_multicast_tbl_size &&  index >= 0 ){
...
int32 rtl8651_setAsicIpMulticastTable(rtl865x_tblAsicDrv_multiCastParam_t *mCast_t, int is_valid) {
	uint32 idx;
#if defined(CONFIG_RTL_8196C_8196E)
	rtl865xc_tblAsic_ipMulticastTable_96e_t entry_96e;
	rtl865xc_tblAsic_ipMulticastTable_96c_t entry_96c;
#else	
	...
	if(IS_RTL8196E)
	{
		bzero(&entry_96e, sizeof(entry_96e));
		entry_96e.srcIPAddr 		= mCast_t->sip;
		entry_96e.destIPAddrLsbs 	= mCast_t->dip & 0xfffffff;
	}
	else
	{
		bzero(&entry_96c, sizeof(entry_96c));
		entry_96c.srcIPAddr 		= mCast_t->sip;
		entry_96c.destIPAddrLsbs 	= mCast_t->dip & 0xfffffff;
	}
#else		
	...
	if(IS_RTL8196C)
	{
		entry_96c.srcPort 			= mCast_t->port;
		entry_96c.portList 			= mCast_t->mbr;
	}
	else
	{
		if (mCast_t->port >= RTL8651_PORT_NUMBER) {
			/* extension port */
			entry_96e.srcPortExt = 1;
			entry_96e.srcPort 			= (mCast_t->port-RTL8651_PORT_NUMBER);
	
		} else {
			entry_96e.srcPortExt = 0;
			entry_96e.srcPort 			= mCast_t->port;
	
		}
	
		entry_96e.extPortList 		= mCast_t->mbr >> RTL8651_PORT_NUMBER;
		entry_96e.srcVidH 			= ((mCast_t->svid)>>4) &0xff;
		entry_96e.srcVidL 			= (mCast_t->svid)&0xf;
		entry_96e.portList 			= mCast_t->mbr & (RTL8651_PHYSICALPORTMASK);
	}
	...
	{
		entry_96e.toCPU 			= 0;
		entry_96e.valid 			= is_valid;
		entry_96e.extIPIndex 		= mCast_t->extIdx;

		entry_96e.ageTime			= 0;
		age = (int16)mCast_t->age;
		while ( age > 0 ) {
			if ( (++entry_96e.ageTime) == 7)
				break;
			age -= 5;
		}
#ifdef CONFIG_HW_MULTICAST_TBL
		...
		return _rtl8651_forceAddAsicEntry(TYPE_MULTICAST_TABLE, idx, &entry_96e);
	
	}
	else
	{
		entry_96c.toCPU 			= 0;
		entry_96c.valid 			= is_valid;
		entry_96c.extIPIndex 		= mCast_t->extIdx;

		entry_96c.ageTime			= 0;
		entry_96e.ageTime			= 0;
		age = (int16)mCast_t->age;
		while ( age > 0 ) {
			if ( (++entry_96c.ageTime) == 7)
				break;
			age -= 5;
		}
	}
	...
	return _rtl8651_forceAddAsicEntry(TYPE_MULTICAST_TABLE, idx, &entry_96c);
...
int32 rtl8651_delAsicIpMulticastTable(uint32 index) {
	...
	{
		bzero(&entry_96e, sizeof(entry_96e));
		return _rtl8651_forceAddAsicEntry(TYPE_MULTICAST_TABLE, index, &entry_96e);
	}
	else
	{
		bzero(&entry_96c, sizeof(entry_96c));
		return _rtl8651_forceAddAsicEntry(TYPE_MULTICAST_TABLE, index, &entry_96c);
	}
#else	
	rtl865xc_tblAsic_ipMulticastTable_t entry;
...
int32 rtl8651_getAsicIpMulticastTable(uint32 index, rtl865x_tblAsicDrv_multiCastParam_t *mCast_t) {	
...
#if defined(CONFIG_RTL_8196C_8196E)
	static rtl865xc_tblAsic_ipMulticastTable_96e_t entry_96e;
	static rtl865xc_tblAsic_ipMulticastTable_96c_t entry_96c;
#else
...
#if defined(CONFIG_RTL_8196C_8196E)
	if(IS_RTL8196E)
		_rtl8651_readAsicEntry(TYPE_MULTICAST_TABLE, index, &entry_96e);
	else
		_rtl8651_readAsicEntry(TYPE_MULTICAST_TABLE, index, &entry_96c);
#else
	...
	if(IS_RTL8196E)
	{
		mCast_t->sip	= entry_96e.srcIPAddr;
 		mCast_t->dip	= entry_96e.destIPAddrLsbs | 0xe0000000;
	}
	else
	{
		mCast_t->sip	= entry_96c.srcIPAddr;
 		mCast_t->dip	= entry_96c.destIPAddrLsbs | 0xe0000000;
	}
#else	
	...
	{
		mCast_t->svid = 0;
		mCast_t->port = entry_96c.srcPort;
		mCast_t->mbr = entry_96c.portList;
	}
	else
	{
		mCast_t->svid = (entry_96e.srcVidH<<4) | entry_96e.srcVidL;

		if (entry_96e.srcPortExt) {
			mCast_t->port = entry_96e.srcPort + RTL8651_PORT_NUMBER;
		} else {
			mCast_t->port = entry_96e.srcPort;
		}
	
		mCast_t->mbr = entry_96e.extPortList<<RTL8651_PORT_NUMBER | entry_96e.portList;
	}
#elif defined(CONFIG_RTL8196C_REVISION_B) || defined(CONFIG_RTL8198) || defined(CONFIG_RTL_8196D)
	mCast_t->svid = 0;
	...
	if(IS_RTL8196E)
	{
		mCast_t->extIdx = entry_96e.extIPIndex ;
		mCast_t->age	= entry_96e.ageTime * 5;
		mCast_t->cpu = entry_96e.toCPU;
	}
	else
	{
		mCast_t->extIdx = entry_96c.extIPIndex ;
		mCast_t->age	= entry_96c.ageTime * 5;
		mCast_t->cpu = entry_96c.toCPU;
	}
#else
...
#if 1
static uint64 rtl865xC_returnAsicCounter64(uint32 offset)
{
	if ( offset & 0x3 )
		return 0;
...
void rtl865xC_dump_vlan(void)
{
		...
		for ( i = 0; i < RTL865XC_VLANTBL_SIZE; i++ )		// CONFIG_RTL_8196D
		{
	...
	again:
			...
			for( ; start<=end; start++) {
				...
				case RTL8651_ACL_SRCFILTER:
					//panic_printk("   ==> rtl865xC_dump_acl: RTL8651_ACL_SRCFILTER\n");
...
int8 rtl8651_tblAsicDrv_Id[] = "$Id: rtl865xC_tblAsicDrv.c,v 1.19.2.5 2010/07/06 03:26:37 keith_huang Exp $";
