/*
  ...
  *
  *	$Id: eth865x.c,v 1.9 2016/05/03 11:22:32 rtlwl Exp $
  *
...
#if defined(DYNAMIC_ADJUST_TASKLET) || defined(CONFIG_RTL8186_TR) || defined(CONFIG_RTL8196B_TR) || defined(BR_SHORTCUT) || defined(CONFIG_RTL865X_AC) || defined(CONFIG_RTL865X_KLD) || defined(CONFIG_RTL8196B_KLD)  || defined(CONFIG_RTL8196C_REVISION_B) || defined(CONFIG_RTL8196C_KLD)|| defined(CONFIG_RTL8198) || defined(CONFIG_RTL8196C_EC) || defined(CONFIG_RTL_8196D) || defined(CONFIG_RTL_8196E) || defined(CONFIG_RTL_8196C_8196E)
...
#endif
#if defined(CONFIG_RTL865X_AC) || defined(CONFIG_RTL865X_KLD) || defined(CONFIG_RTL8196B_KLD) || defined(CONFIG_RTL8196C_KLD) || defined(CONFIG_RTL_8196E_KLD)
    unsigned int    tx_avarage;
    unsigned int    tx_peak;
    unsigned int    rx_avarage;
    unsigned int    rx_peak;
...
static struct init_vlan_setting vlanSetting_96c[] = {
...
static struct init_vlan_setting vlanSetting_96e[] = {
...
static struct init_vlan_setting vlanSetting[] = {
    ...
    {LAN4_VID,  BIT(4),                               {0x00, 0xe0, 0x4c, 0xf0, 0x00, 0x04} },  // eth4(LAN) for Guest Zone
#endif    
#endif//#if defined(CONFIG_RTK_GUEST_ZONE)
#if defined(CONFIG_RTK_VLAN_SUPPORT)
#if defined(CONFIG_RTL8196C) || defined(CONFIG_RTL8198) || defined(CONFIG_RTL_8196E)
    {LAN2_VID,  BIT(1),                               {0x00, 0xe0, 0x4c, 0xf0, 0x00, 0x02} },  // eth2(LAN) for Guest Zone
    {LAN3_VID,  BIT(2),                               {0x00, 0xe0, 0x4c, 0xf0, 0x00, 0x03} },  // eth3(LAN) for Guest Zone
    {LAN4_VID,  BIT(3),                               {0x00, 0xe0, 0x4c, 0xf0, 0x00, 0x04} },  // eth4(LAN) for Guest Zone
...
#if defined(CONFIG_RTL_8196C_8196E)
static unsigned int totalVlans_96c = sizeof(vlanSetting_96c)/sizeof(struct init_vlan_setting);
static unsigned int totalVlans_96e = sizeof(vlanSetting_96e)/sizeof(struct init_vlan_setting);
#else
...
static unsigned char prev_port_sts[MAX_PORT_NUMBER];
#endif
...
int is_MulticastData(struct sk_buff *skb)
{
	...
	{
		if( V4_IGMP_PROTO(dest) || V6_DHCPV6_PROTO(dest) || V6_ICMPV6_PROTO(dest) || SSDP_PROTO(dest) )	
			return 0;
		else
			return 1;	
	}else
...
#ifdef CONFIG_EFM_PATCH
static const unsigned char mDNS_mc_addr[6] = {0x01, 0x00, 0x5e, 0x00, 0x00, 0xfb};
static const unsigned char SSDP_mc_addr[6] = {0x01, 0x00, 0x5e, 0x7f, 0xff, 0xfa};
...
#endif		
#ifdef CONFIG_EFM_PATCH
                &&(eth_flag & BIT(3)) /* if multicast route is enabgled , */
#endif
	)
	{
		/*process eth port */
		...
		//printk("indicate_rx -> %02x %02x %02x %02x %02x %02x : %s\n", skb->data[0],skb->data[1],skb->data[2],skb->data[3],skb->data[4],skb->data[5], skb->dev->name); 
...
static void interrupt_dsr_rx(unsigned long task_priv)
{
	...
	if (L2_table_disabled) {		
		...
		if  (memcmp(&skb->data[6], cp_this->dev->dev_addr, 6)== 0)// check source mac
		{
                ...
                {
                         if(eth_flag & BIT(3)) /* if multicast route is enabgled , */
                         {
                                 if (
                                     (memcmp(&skb->data[0], mDNS_mc_addr, 6) == 0) ||
                                     (memcmp(&skb->data[0], SSDP_mc_addr, 6) == 0) ||
                                     /*added from realtek's f patch */
                                     ...
                                     )
                                 {
                                         ...
                                         if (newskb) {
                                                 newskb->cb[0] = 0x1f & (~BIT(wan_port)) & ~(pid);
                                                 newskb->cb[3] = 0;
                                                 //printk("Forward Multicast --> %02x:%02x:%02x:%02x:%02x:%02x\n", skb->data[0], skb->data[1], skb->data[2], skb->data[3], skb->data[4], skb->data[5] );
                                                 swNic_send((void *)newskb, newskb->data, len, vid, newskb->cb[0]);
                                         }
                                 }
#ifndef CONFIG_RTK_VLC_SPEEDUP_SUPPORT
// ysyoo BEGIN 2013-09-06
                                 else if(V4_MULTICAST_MAC(skb->data))
                                 {
                                         struct sk_buff *newskb;
                                         newskb = skb_clone(skb, GFP_ATOMIC);
                                         if (newskb) {
						// bit0:wan, bit1:user_port4
                                                 newskb->cb[0] = 0x2 & (~BIT(wan_port)) & ~(pid);
                                                 newskb->cb[3] = 0;
                                                 swNic_send((void *)newskb, newskb->data, len, vid, newskb->cb[0]);
                                         }
                                 }
// ysyoo END
                         ...
                         /* if IGMP proxy is off,  flood IGMP packet */
                         if(V4_MULTICAST_MAC(skb->data) && V4_IGMP_PROTO(skb->data))
                         {
                                 struct sk_buff *newskb;
                                 newskb = skb_clone(skb, GFP_ATOMIC);
                                 if (newskb) {
...
rx_packet:
	...
	if(savOP_MODE_value == 2 && *(unsigned short *)(skb->data+12) == 0x86dd ) // gw mode
	{
		if(pid == BIT(wan_port)){	// packet come from wan port
		struct net_device *disg_dev = NULL; 

		#if defined(CONFIG_RTK_VLAN_SUPPORT) || defined(CONFIG_RTK_GUEST_ZONE)
...
static void interrupt_isr_default(int irq, void *dev_instance, struct pt_regs *regs)
{
...
#if defined(CONFIG_RTL8196C_ETH_IOT)
    if (status & LINK_CHANGE_IP) {
...
#if defined(CONFIG_RTL_8196C_8196E)
static void interrupt_isr_96e(int irq, void *dev_instance, struct pt_regs *regs)
{
    struct net_device *dev = dev_instance;
    struct re_private *cp = dev->priv;
	unsigned int status;
...
static struct net_device_stats *rtl865x_get_stats(struct net_device *dev)
{
	...
	if (gHwNatEnabled > 0) {
		...
		for (vid=0; vid<2; vid++) {
			...
			if(IS_RTL8196E)
			{
				if (!memcmp(vlanSetting_96e[vid].mac, dev->dev_addr, 6))
					break;
			}
			else
			{
				if (!memcmp(vlanSetting_96c[vid].mac, dev->dev_addr, 6))
					break;
			}
#else			
		...
		for (port=0; port<MAX_PORT_NUMBER; port++) {
			...
			int portmask_val;
			if(IS_RTL8196E)
				portmask_val = vlanSetting_96e[vid].portmask
			else
				portmask_val = vlanSetting_96c[vid].portmask
#else
			portmask_val = vlanSetting[vid].portmask
#endif
				
			if 	(portmask_val & (1<<port)) {
...
#if defined(CONFIG_RTL8196C) || defined(CONFIG_RTL8198) || defined(CONFIG_RTL_8196C_8196E)/* only support in 8196c	*/
        ...
        for (port=0; port<5; port++)
        {
                ...
                if (AutoDownSpeed_10M[port] == 0x12345678) {
                        ...
                        if ((!curr_sts) && (ReverSpeed_flag[port]==1)) {
                                ...
                                //printk(" => check_cb_snr: port= %d, ph1\n", port);
                        }
                ...
                if ((curr_sts==1) && (!link_speed_10M)) {
                        ...
                        if (match_count[port] >= 5) {
                                ...
                                ReverSpeed_flag[port]=0;
                                match_count[port] = 0;
                                //printk("Down speed to 10M for port %d\n", port);
                        }
                }
        }
        return 0;
...
#if defined(DYNAMIC_ADJUST_TASKLET) || defined(CONFIG_RTL8186_TR) || defined(CONFIG_RTL8196B_TR) || defined(BR_SHORTCUT) || defined(CONFIG_RTL865X_AC) || defined(CONFIG_RTL865X_KLD) || defined(CONFIG_RTL8196B_KLD)  || defined(CONFIG_RTL8196C_REVISION_B) || defined(CONFIG_RTL8198) /*|| defined(CONFIG_RTK_MESH)*/ || defined(CONFIG_RTL8196C_EC) || defined(CONFIG_RTL_8196D) || defined(CONFIG_RTL_8196E) || defined(CONFIG_RTL_8196C_8196E)
__CE_MISS
...
*/
	...
	extern unsigned int isRTL8196C_REV_A;
	if (isRTL8196C_REV_A && (eee_enabled)) {
		int i, curr_sts;
		uint32 reg;
...
#if defined(CONFIG_RTL_8196C_ESD_NEW) || defined(CONFIG_RTL_8196C_8196E)
	if(is8196C)
#ifdef CONFIG_EFM_PATCH
                if_check = ((((REG32(PCRP4) & EnablePHYIf) == 0) && !diag_status)?1:0);
#else
		if_check = ((REG32(PCRP4) & EnablePHYIf) == 0?1:0);
#endif
	else
		if_check = check_tx_desc_hang();
#else
	if_check = check_tx_desc_hang();
#endif
...
#endif	//#ifdef CONFIG_RTL_8196C_ESD_NEW
		hangup_reinit = 1;
...
#endif //#ifdef CONFIG_RTK_CHECK_ETH_TX_HANG

	mod_timer(&cp->expire_timer, jiffies + 100);	
#else //#ifdef ETH_NEW_FC
	mod_timer(&cp->expire_timer, jiffies + 100);
...
#if defined(CONFIG_RTL8196C_GREEN_ETHERNET) || defined(CONFIG_RTL_8196C_8196E)
static void power_save_timer(unsigned long task_priv)
{
	if(IS_RTL8196C)
	{
#if defined(CONFIG_RTL8196C_GREEN_ETHERNET)	
		uint32 flags;
	  struct re_private *cp = ((struct net_device *)task_priv)->priv;
		...
		restore_flags(flags);			
#endif //#if defined(CONFIG_RTL8196C_GREEN_ETHERNET)
	}
}
#endif
...
static int rtl865x_set_hwaddr(struct net_device *dev, void *addr)
{
...
#if defined(CONFIG_RTL_8196C_8196E)
		if(IS_RTL8196E)
			memcpy(vlanSetting_96e[i].mac, dev->dev_addr, 6);
		else
			memcpy(vlanSetting_96c[i].mac, dev->dev_addr, 6);
	...
	if (i == 0) {
		...
		if(IS_RTL8196E)
		{
			memcpy(vlanSetting_96e[2].mac, dev->dev_addr, 6);
			memcpy(vlanSetting_96e[3].mac, dev->dev_addr, 6);
			memcpy(vlanSetting_96e[4].mac, dev->dev_addr, 6);
		}
		else
		{
			memcpy(vlanSetting_96c[2].mac, dev->dev_addr, 6);
			memcpy(vlanSetting_96c[3].mac, dev->dev_addr, 6);
			memcpy(vlanSetting_96c[4].mac, dev->dev_addr, 6);
		}
#else				
	...
	if(IS_RTL8196E)
		totalVlans_val = totalVlans_96e;
	else
		totalVlans_val = totalVlans_96c;
#else
	totalVlans_val = totalVlans;
#endif	
	swCore_init(-1); // reset interface table
	for (i=0; i<totalVlans_val; i++) {    
		bzero((void *) &np, sizeof(rtl_netif_param_t));
#if defined(CONFIG_RTL_8196C_8196E)
		if(IS_RTL8196E)
			np.vid = vlanSetting_96e[i].vid;
		else
			np.vid = vlanSetting_96c[i].vid;
#else		
...
#if defined(CONFIG_RTL_8196C_8196E)
		if(IS_RTL8196E)
			memcpy(&np.gMac, &vlanSetting_96e[i].mac[0], 6);                        
		else
			memcpy(&np.gMac, &vlanSetting_96c[i].mac[0], 6);
...
static int rtl865x_init_hw(void)
{
	...
	if(IS_RTL8196E)
	{
		setting = vlanSetting_96e;
		totalVlans_val = totalVlans_96e;
	}
	else
	{
		setting = vlanSetting_96c;
		totalVlans_val = totalVlans_96c;
	}
#else
	setting = vlanSetting;
	...
	for (ethno = 0; ethno < totalVlans_val; ethno++) {        
		/* Create NetIF */
		bzero((void *) &np, sizeof(rtl_netif_param_t));
	...
	if ((eth_flag & BIT(3)) && (savOP_MODE_value==2))
		...
		REG32(FFCR) = REG32(FFCR)  & ~IPMltCstCtrl_Enable;		
#endif //#if defined(CONFIG_RTL8196C_AP_ROOT)
#endif
...
#if defined(CONFIG_RTL_8196C_8196E)
		if(IS_RTL8196E)
			macPtr = vlanSetting_96e[vid].mac;
		else
			macPtr = vlanSetting_96c[vid].mac;
#else
		macPtr = vlanSetting[vid].mac;
#endif

		if (!memcmp(macPtr, dev->dev_addr, 6))
			break;
	}
	if (vid == 2) {
...
#if defined(CONFIG_RTL_8196C_8196E)
		if(IS_RTL8196E)
            rc = request_irq(dev->irq, interrupt_isr_96e, SA_INTERRUPT, dev->name, dev);
		else
#endif
			rc = request_irq(dev->irq, interrupt_isr_default, SA_INTERRUPT, dev->name, dev);
		if (rc) {
...
#if defined(DYNAMIC_ADJUST_TASKLET) || defined(BR_SHORTCUT)  || defined(CONFIG_RTL8196C_REVISION_B)|| defined(CONFIG_RTL8198) || defined(CONFIG_RTL_8196D) || defined(CONFIG_RTL_8196E) || defined(CONFIG_RTL_8196C_8196E)
...
#if defined(DYNAMIC_ADJUST_TASKLET) || defined(CONFIG_RTL8186_TR) || defined(CONFIG_RTL8196B_TR) || defined(BR_SHORTCUT) || defined(CONFIG_RTL865X_AC) || defined(CONFIG_RTL865X_KLD) || defined(CONFIG_RTL8196B_KLD) ||defined(CONFIG_RTL8196C_REVISION_B) || defined(CONFIG_RTL8196C_KLD)|| defined(CONFIG_RTL8198) || defined(CONFIG_RTL_8196D) || defined(CONFIG_RTL_8196E) || defined(CONFIG_RTL_8196C_8196E)
#ifdef ETH_NEW_FC
#ifdef CONFIG_RTK_CHECK_ETH_TX_HANG	
	if (!hangup_reinit)
#endif		
...
#if defined(CONFIG_RTL8196C) || defined(CONFIG_RTL_8196E) || defined(CONFIG_RTL_8196C_8196E)
        if(IS_RTL8196E)
        {
                unsigned int val;
                val = (REG32(BOND_OPTION) & BOND_ID_MASK);
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		...
		{			
			...
			if (op_mode == 0) {
				...
				if(IS_RTL8196E)
				{
					vlanSetting_96e[0].portmask = ALL_PORTS & (~BIT(wan_port));
					vlanSetting_96e[1].portmask = BIT(wan_port);
				}
				else
				{
					vlanSetting_96c[0].portmask = ALL_PORTS & (~BIT(wan_port));
					vlanSetting_96c[1].portmask = BIT(wan_port);
				}
#else				
				...
				if(IS_RTL8196E)
				{
					vlanSetting_96e[0].portmask = ALL_PORTS;
					vlanSetting_96e[1].portmask = 0;
				}
				else
				{
					vlanSetting_96c[0].portmask = ALL_PORTS;
					vlanSetting_96c[1].portmask = 0;
				}
#else								
...
#if defined(CONFIG_RTL_8196C_8196E)
			if(IS_RTL8196E)
				cp->port = vlanSetting_96e[0].portmask;
			else
				cp->port = vlanSetting_96c[0].portmask;
#else			
...
#if defined(CONFIG_RTL_8196C_8196E)
			if(IS_RTL8196E)
				cp->port = vlanSetting_96e[1].portmask;
			else
				cp->port = vlanSetting_96c[1].portmask;
#else						
...
int dbg_cmd = 0, dbg_len=16;
uint32 dbg_addr = 0xb8000000;
int rtl865xC_dumpAsicCounter2(char *page, int len);
		     ...
		     int count, int *eof, void *data)
{
	...
	len = sprintf(page, " \n");
        ...
        if (dbg_cmd == 1) {
                len = rtl865xC_dumpAsicCounter2(page, len);
        }
        else if (dbg_cmd == 2) {
                int i, len2 = dbg_len;
                uint32 addr = dbg_addr;
                if (len2 < 16) len2 = 16;
                len2 -= (len2 % 4);
                len2 = len2 >> 4;
                ...
                for (i=0; i<len2; i++)
                {
                        len += sprintf(page+len,"  0x%x:\t0x%08x\t0x%08x\t0x%08x\t0x%08x\n", addr,
			...
			if (!memcmp(tokptr, "desc", 4)) {
               ...
               else if (!memcmp(cmd_addr, "set", 3))
                {
                        ...
                        if (!memcmp(tokptr, "time5", 5)) {
                                ...
                                if (tokptr==NULL)
                                        goto errout;
                                dbg_cmd=(int)simple_strtoul(tokptr, NULL, 0);

                                tokptr = strsep(&strptr," ");
                                if (tokptr==NULL)
                                        goto errout;
                                dbg_addr=(uint32)simple_strtoul(tokptr, NULL, 16);

                                tokptr = strsep(&strptr," ");
                                if (tokptr==NULL)
                                        goto errout;
                                dbg_len=(int)simple_strtoul(tokptr, NULL, 0);

                                panic_printk("  command success. dbg_cmd= %d, dbg_addr= 0x%x, dbg_len= %d\n",
                                        dbg_cmd, dbg_addr, dbg_len);                                                             
                        }
...
#ifdef CONFIG_EFM_PATCH
static struct proc_dir_entry *proc_diag_status=NULL;
static struct proc_dir_entry *proc_chip_name=NULL;
static int write_proc_diag_status(struct file *file, const char *buffer,
                      unsigned long count, void *data)
{
        unsigned char tmp[4];
        uint32 statCtrlReg, phyId; // = wan_port;
...
static int read_proc_chip_name(char *page, char **start, off_t off,
                     int count, int *eof, void *data)
{
        int len;
        len = sprintf(page, "%s\n", IS_RTL8196E?"RTL8196E":"RTL8196C");
        if (len <= off+count)
                *eof = 1;
...
static int __init rtl865x_probe(int ethno)
{
...
#if defined(CONFIG_RTL_8196C_8196E)
		if(IS_RTL8196E)
			((u8 *)(dev->dev_addr))[i] =vlanSetting_96e[ethno].mac[i];
		else
			((u8 *)(dev->dev_addr))[i] =vlanSetting_96c[ethno].mac[i];
	...
	if(IS_RTL8196E)
	{
		cp->vid = vlanSetting_96e[ethno].vid;
		cp->port = vlanSetting_96e[ethno].portmask; 
	}
	else
	{
		cp->vid = vlanSetting_96c[ethno].vid;
		cp->port = vlanSetting_96c[ethno].portmask; 
	}
#else	
...
#if	1
 		{
			static struct proc_dir_entry *mem_entry;

                        mem_entry = create_proc_entry("memory",0,NULL);
                        if(mem_entry != NULL)
                        {
...
int set_port_led(int flag)
{
        ...
        else
        {
                ...
                if(IS_RTL8196E)
                {
                        printk("set_port_led : %08x to 0x3FF\n",  REG32(PIN_MUX_SEL2));
                        REG32(PIN_MUX_SEL2) |= 0x3FFF;
                }
                else
#endif
...
static int32 proc_led_write( struct file *filp, const char *buff,unsigned long len, void *data )
{
        char            tmpbuf[64];
...
{ 
...
#ifdef CONFIG_EFM_PATCH
        proc_diag_status = create_proc_entry("diag_status", 0, NULL);
        if (proc_diag_status) {
                proc_diag_status->write_proc = write_proc_diag_status;
        }

        rtl865x_proc_dir = proc_mkdir(RTL865X_PROC_DIR_NAME,NULL);
        led_entry = create_proc_entry("led",0,rtl865x_proc_dir);
        if(led_entry)
                led_entry->write_proc= proc_led_write;

        proc_chip_name = create_proc_entry("chip_name", 0, NULL);
        if (proc_chip_name) {
                proc_chip_name->read_proc = read_proc_chip_name;
        }
	...
	if(IS_RTL8196E)
	{
		vlanSetting_96e[0].portmask = 0;
		vlanSetting_96e[1].portmask=BIT(wan_port);
	}
	else
	{
		vlanSetting_96c[0].portmask = 0;
		vlanSetting_96c[1].portmask=BIT(wan_port);
	}
#else	
...
void add_WAN_VLAN(unsigned int nvid){
	...
	if(IS_RTL8196E)
	{
		vp.egressUntag = vlanSetting_96e[1].portmask; //vlanSetting[1].portmask;
		vp.memberPort = vlanSetting_96e[1].portmask;
	}
	else
	{
		vp.egressUntag = vlanSetting_96c[1].portmask; //vlanSetting[1].portmask;
		vp.memberPort = vlanSetting_96c[1].portmask;
	}
#else	
...
void del_WAN_VLAN(unsigned short nvid){
	...
	if(IS_RTL8196E)
	{
		if( nvid != vlanSetting_96e[1].vid && nvid != vlanSetting_96e[0].vid)
			swCore_vlanDestroy(nvid);
	}
	else
	{
		if( nvid != vlanSetting_96c[1].vid && nvid != vlanSetting_96c[0].vid)
			swCore_vlanDestroy(nvid);
	}
#else
...
#if defined(CONFIG_RTL_8196C_8196E)
	if(IS_RTL8196E)
		totalVlans_96e = count;
	else
		totalVlans_96c = count;
#else
   totalVlans = count;
#endif    	
...
#if defined(CONFIG_RTL_8196C_8196E)
		if(IS_RTL8196E)
			totalVlans_96e = ETH_INTF_NUM;
		else
			totalVlans_96c = ETH_INTF_NUM;
#else		
