			       ...
			       int (*okfn)(struct sk_buff *))
{
	for (*i = (*i)->next; *i != head; *i = (*i)->next) {
		...
		//printk("elem->hook=%08x\n", (unsigned int)(elem->hook));
#endif
		 ...
		 int (*okfn)(struct sk_buff *))
{
	...
	printk("1: nf_hook_slow: saddr=%08X, skb=%08x\n",iph->saddr, (unsigned int)skb);
#endif
...
#ifdef JACKSON_NET_TRACE	
	printk("2: nf_hook_slow: saddr=%08X, skb=%08x,verdict=%08x\n",iph->saddr, (unsigned int)skb,verdict);
#endif
	return ret;
}
