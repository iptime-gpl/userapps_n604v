 ...
 *
 *	Version:	$Id: skbuff.c,v 1.3 2014/08/07 05:13:35 rtlwl Exp $
 *
 *	Fixes:	
...
extern int (*wirelessnet_hook_is_priv_buf)(unsigned char *head);
extern void (*wirelessnet_hook_free_priv_buf)(unsigned char *head);

EXPORT_SYMBOL(wirelessnet_hook_is_priv_buf);
EXPORT_SYMBOL(wirelessnet_hook_free_priv_buf);
int (*wirelessnet_hook_is_priv_buf)(unsigned char *head)=NULL;
void (*wirelessnet_hook_free_priv_buf)(unsigned char *head)=NULL;

#else // !CONFIG_WIRELESS_LAN_MODULE
...
struct sk_buff *alloc_skb(unsigned int size,int gfp_mask)
{
	...
	size = SKB_DATA_ALIGN(size + 16);
	#else
	//sc_yang : Add addition 8 bytes for rtl8186 tkip
	size = SKB_DATA_ALIGN(size + 8);
	#endif
#endif
	...
	skb->head = data;
#ifdef	DRIVER_SPEEDUP 
	//skb->data = (unsigned char *)((unsigned int)data | 0xa0000000);
	skb->data = data;
#else
	skb->data = data;
#endif
	...
	skb->rx_port = 1<<6; //default: Local generate
#endif
...
#ifdef CONFIG_RTL8190_PRIV_SKB
#ifdef CONFIG_WIRELESS_LAN_MODULE
		if((wirelessnet_hook_is_priv_buf!= NULL) && (wirelessnet_hook_free_priv_buf!=NULL) )
		{
			if (wirelessnet_hook_is_priv_buf(skb->head)) 
			wirelessnet_hook_free_priv_buf(skb->head);
		}
		else
#else
