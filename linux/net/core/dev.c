...
#if defined(NAT_SPEEDUP) || defined(CONFIG_RTK_IPTABLES_FAST_PATH)
	#define FAST_PPTP
	#define FAST_L2TP
#endif	

#ifdef FAST_L2TP
	void (*l2tp_tx_id_hook)(struct sk_buff *skb) = NULL;			
#ifdef CONFIG_FAST_PATH_MODULE
...
//mark_add ,for time.c
#define RTK_RX_TIME_SQUEEZE_CHECK 1
#ifdef RTK_RX_TIME_SQUEEZE_CHECK
unsigned int rtk_rx_time_squeeze=0; //rx packet process beyond 1 tick(it 's only for single cpu)!
#endif
...
#ifdef CONFIG_RTL_KERNEL_MIPS16_NETCORE
void dev_load(const char *unused){;}
#else
...
#ifdef JACKSON_NET_TRACE

			//printk("At%08x: ptype->func=%08x\n", jiffies, (unsigned int)ptype->func);
#endif			
...
#endif	
#ifdef FAST_PPTP
	else if (sync_tx_pptp_gre_seqno_hook != NULL)			
		sync_tx_pptp_gre_seqno_hook(skb);			
#endif	
#ifdef CONFIG_RTL865X_HW_PPTPL2TP
	{
		extern int rtl865x_acceleratePptpToWanSync(struct sk_buff *skb);
...
//RTK:roll back to original kernel behavior
//#ifndef CONFIG_RTK_VOIP
//#define JACKSON_NETPLUS
//#endif

#ifdef JACKSON_NETPLUS
		netif_schedule(dev);
...
int netdev_max_backlog = 300; // QoS in VoIP
#else
int netdev_max_backlog = 1300; // after smart bit test, 800 is optimized
#endif
...
int netif_rx_voip(struct sk_buff *skb)
{
	int this_cpu = smp_processor_id();
	struct softnet_data *queue;
	unsigned long flags;
	...
	netdev_rx_stat[this_cpu].total++;
	if (queue->input_pkt_queue_voip.qlen <= netdev_max_backlog) {
		if (queue->input_pkt_queue_voip.qlen) {
			if (queue->throttle)
				;//goto drop;
enqueue:
			dev_hold(skb->dev);
			__skb_queue_tail(&queue->input_pkt_queue_voip,skb);
			/* Runs from irqs or BH's, no need to wake BH */
			cpu_raise_softirq(this_cpu, NET_RX_SOFTIRQ);
			local_irq_restore(flags);
#ifndef JACKSON_NET_WORK			
...
#if defined(CONFIG_NAT_SPEEDUP_MODULE)
struct sk_buff* (*nat_speedup_hook)(struct sk_buff_head* queue) = NULL;
#endif
//--------------------------------
...
#ifdef NAT_SPEEDUP
static void cheatsend(struct sk_buff* skb)
{
...
static void net_rx_action(struct softirq_action *h)
{
...
#ifdef NAT_SPEEDUP
#ifdef CONFIG_NAT_SPEEDUP_MODULE
	if (nat_speedup_hook != NULL) {		
		skb = nat_speedup_hook(&queue->input_pkt_queue);
		if (skb == NULL)
	      break;			
	}		
	else {	
...
#else // !CONFIG_NAT_SPEEDUP_MODULE
	skb = NatSpeedUpTunnel(&queue->input_pkt_queue);			
	...
	if (skb == NULL)
		...
		local_irq_disable();
#ifdef SUPPORT_INPUT_PKT_QUEUE_VOIP
		skb = __skb_dequeue(&queue->input_pkt_queue_voip);
		if (skb == NULL)
#endif			
		skb = __skb_dequeue(&queue->input_pkt_queue);
		local_irq_enable();
...
#ifdef CONFIG_RTL865X_HW_PPTPL2TP
		if (rtl865x_acceleratePptpL2tp(&skb) == NET_RX_DROP) {
			/* Accelerated, process next. */
			continue;	
		}	
#endif
#ifdef	CONFIG_RTK_IPTABLES_FAST_PATH
		#ifdef CONFIG_FAST_PATH_MODULE
		if(fast_path_hook!=NULL)
		{
			if (fast_path_hook(&skb)== NET_RX_DROP) 	
			continue;
...
extern unsigned char wan_if[];
        if(strlen(wan_if) != 0 && skb->protocol == 0x86dd) // 86dd:ipv6 protocol type
  {
                if (strcmp(skb->dev->name,wan_if) == 0)
                {
#if defined(CONFIG_RTK_VLAN_SUPPORT) && !defined(CONFIG_RTK_VLAN_ROUTETYPE)
                        skb->dev = dev_get_by_name("eth5");
#elif defined(CONFIG_RTK_VLAN_SUPPORT) && defined(CONFIG_RTK_VLAN_ROUTETYPE)
                        skb->dev = dev_get_by_name("eth6");
#else
                        skb->dev = dev_get_by_name("eth2");
#endif
                        if(skb->dev == NULL)
                        {
panic_printk("\r\n IPv6 fault!!__[%s-%u]\r\n",__FILE__,__LINE__);
...
#ifdef RTK_RX_TIME_SQUEEZE_CHECK	
	rtk_rx_time_squeeze = netdev_rx_stat[this_cpu].time_squeeze; //mark_add
#endif	
...
#ifndef CONFIG_WIRELESS_EXT_V18
//old wireless extension v12
#ifdef WIRELESS_EXT
#ifdef CONFIG_PROC_FS
...
#ifdef SUPPORT_INPUT_PKT_QUEUE_VOIP
		skb_queue_head_init(&queue->input_pkt_queue_voip);
#endif
		queue->throttle = 0;
		queue->cng_level = 0;
