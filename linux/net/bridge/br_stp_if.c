 ...
 *
 *	$Id: br_stp_if.c,v 1.2 2010/06/28 02:07:45 ysrtl Exp $
 *
...
void br_stp_disable_port(struct net_bridge_port *p)
{
	...
	if (p->disable_by_mesh == 0)
		...
		br_timer_set(&p->eth_disable_timer, jiffies);
	}
#endif
#endif
...
/* called under bridge lock */
void br_stp_assign_bridge_id(struct net_bridge *br, unsigned char *addr)
{
	struct net_bridge_port *p;

	p = br->port_list;
	while (p != NULL) {
		if (!memcmp(p->dev->dev_addr, addr, ETH_ALEN))
			break;
	...
	if (p == NULL) {
		printk(KERN_INFO "%s: set address failed!\n", br->dev.name);
		return;
	}
