...
extern int ip_nat_seq_adjust(struct sk_buff *skb, struct ip_conntrack *ct, int ctinfo);
extern int __ip_conntrack_confirm(struct nf_ct_info *nfct);
extern struct sk_buff* (*nat_speedup_hook)(struct sk_buff_head* queue);
...
EXPORT_SYMBOL(unknown_nat_protocol);
EXPORT_SYMBOL(protos);
EXPORT_SYMBOL(ip_nat_seq_adjust);
EXPORT_SYMBOL(__ip_conntrack_confirm);
EXPORT_SYMBOL(nat_speedup_hook);
...
extern void (*twinip_tx_handler)(struct sk_buff *skb);
EXPORT_SYMBOL(twinip_rx_handler);
EXPORT_SYMBOL(twinip_tx_handler);
#endif
...
EXPORT_SYMBOL(multicast_forward_handler);
#endif
