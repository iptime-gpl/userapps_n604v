 ...
 *
 * Version:	$Id: ip_output.c,v 1.2 2010/06/28 02:13:23 ysrtl Exp $
 *
...
#ifdef JACKSON_NET_TRACE
		//printk("4: For ip_output: saddr=%08x\n",iph->saddr); 
		//printk("4:At %08x  Athh->hh_output=%08x\n", jiffies, (unsigned int)(hh->hh_output));
#endif
		return hh->hh_output(skb);
	} else if (dst->neighbour)
#ifdef JACKSON_NET_TRACE
	{
		printk("dst->neighbour->output=%08x\n", (unsigned int)(dst->neighbour->output));
...
#ifdef CONFIG_SKIP_NETFILTER_HOOK_FOR_MULTICAST
	if (MULTICAST(skb->nh.iph->daddr))
                return ip_finish_output2(skb);
#endif
#ifdef CONFIG_PASSTHROUGH_ALL_CHAIN
        if(sysctl_nat_passthrough_flag)
               return ip_finish_output2(skb);
        else
#endif
		  ...
		  int flags)
{
	...
	do {
		...
		skb = sock_alloc_send_skb(sk, fraglen+hh_len+15+128, flags&MSG_DONTWAIT, &err);
#else
...
#ifdef CONFIG_RTL_IPSEC      
	skb = sock_alloc_send_skb(sk, length+hh_len+15+128,
...
#ifdef CONFIG_RTL_IPSEC              
		if ((skb2 = alloc_skb(len+hlen+dev->hard_header_len+15+128,GFP_ATOMIC)) == NULL) {
#else		
...
#ifdef CONFIG_RTL_IPSEC      
		skb_reserve(skb2, (dev->hard_header_len+15+80)&~15);
#else
