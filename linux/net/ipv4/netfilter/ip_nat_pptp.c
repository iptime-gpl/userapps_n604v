...
//				htonl(nat_pptp_info->pac_call_id);
	...
	if (HOOK2MANIP(hooknum) == IP_NAT_MANIP_DST) {
		// +++ Gemtek. PPTP Passthrough from WAN to LAN
...

{
	...
	switch (msg = ntohs(ctlh->messageType)) {
		case PPTP_OUT_CALL_REQUEST:
			...
			DEBUGP("pptp_outbound_pkt: OUT_CALL_REQUEST: pns_call_id -> nat:%d ct:%d\n", nat_pptp_info->pns_call_id, ct_pptp_info->pns_call_id );

			//new_callid = tcph->source;
			new_callid = (unsigned short)ntohl(tcph->seq);
			/* save new call ID in ct info */
			//ct_pptp_info->pns_call_id = htons(new_callid);
			ct_pptp_info->pns_call_id = ntohs(new_callid);
			DEBUGP("pptp_outbound_pkt: new call id : %08x \n", ct_pptp_info->pns_call_id);
			break;
			     ...
			     unsigned int hooknum, struct sk_buff **pskb)
{
	...
	if (!((hooknum == NF_IP_POST_ROUTING && dir == IP_CT_DIR_ORIGINAL)
	      ...
	      || (hooknum == NF_IP_LOCAL_OUT && dir == IP_CT_DIR_REPLY)
	      || (hooknum == NF_IP_LOCAL_IN && dir == IP_CT_DIR_ORIGINAL)
	)) {
	...
	if (datalen < sizeof(struct pptp_pkt_hdr)+
		...
		DEBUGP("pptp packet too short :%d\n", datalen);
		return NF_ACCEPT;	
	}
