			 	  ...
			 	  struct sk_buff **pskb)
{
	...
	struct ip_ct_h225_expect *exp_info;
        ...
        if ((ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.ip & htonl(0xffffff00)) == htonl(0xc0a8ff00))
        {  /* 192.168.255.2 */
                return NF_ACCEPT;
        }
        if ((ct->tuplehash[IP_CT_DIR_REPLY].tuple.src.ip & htonl(0xffffff00))== htonl(0xc0a8ff00))
        {  /* 192.168.255.2 */
                return NF_ACCEPT;
        }
#endif
