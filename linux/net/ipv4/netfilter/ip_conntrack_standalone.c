...
print_conntrack(char *buffer, struct ip_conntrack *conntrack)
{
	...
	len += sprintf(buffer + len, "use=%u ",
        ...
        len += sprintf(buffer + len, " org=%lu:%llu rpl=%lu:%llu ",
                conntrack->tuplehash[IP_CT_DIR_ORIGINAL].packets,
                conntrack->tuplehash[IP_CT_DIR_ORIGINAL].bytes,
                conntrack->tuplehash[IP_CT_DIR_REPLY].packets,
                conntrack->tuplehash[IP_CT_DIR_REPLY].bytes);
#endif

#ifdef CONFIG_IP_NF_NAT_PORT_RESTRICTED_CONE
        len += sprintf(buffer + len, " %s ", (conntrack->port_restricted_cone == 1) ? "[PORT CONE]" : "");
#endif
	len += sprintf(buffer + len, "\n");
...
static inline int
conntrack_iterate_new(const struct list_head  *hash,
		  char *buffer, off_t offset, off_t *upto,
		  unsigned int *len, unsigned int maxlen)
{
	unsigned int newlen;	
	...
	ct = list_entry(hash, struct ip_conntrack, state_tuple);
	newlen = print_conntrack(buffer + *len, ct);
	if (*len + newlen > maxlen)
		return 1;
	else *len += newlen;
...
int list_new_conntracks(char *buffer, char **start, off_t offset, int length)
{
	unsigned int i;
	unsigned int len = 0;
	off_t upto = 0;
//	struct list_head *it;
	...
	for (i = 1; i < TCP_CONNTRACK_MAX; i++) {
		if (LIST_FIND(Tcp_State_Hash_Head[i].state_hash, conntrack_iterate_new, 
			struct list_head *, buffer, offset, &upto, &len, length))
			goto finished;
	}
	
        for (i = 1; i < 2; i++) {
		if (LIST_FIND(Udp_State_Hash_Head[i].state_hash, conntrack_iterate_new,
			      struct list_head *,buffer, offset, &upto, &len, length))
			goto finished;
	}
...
extern int tcp_conntrack_max;
extern int udp_conntrack_max;
extern int icmp_conntrack_max;
extern int max_rate_per_ip;
#endif
...
/* Form ALGs */
	...
	extern int ip_ct_alg_ftp;
	#endif
	#ifdef CONFIG_IP_NF_H323
	extern int ip_ct_alg_h323;
	#endif
	#ifdef CONFIG_IP_NF_AMANDA
	extern int ip_ct_alg_amanda;
	#endif
	#ifdef CONFIG_IP_NF_TFTP
	extern int ip_ct_alg_tftp;
	#endif
	#ifdef CONFIG_IP_NF_IRC
	extern int ip_ct_alg_irc;
	#endif
	#ifdef CONFIG_IP_NF_PPTP
	extern int ip_ct_alg_pptp;
	#endif
	#ifdef CONFIG_IP_NF_IPSEC
	extern int ip_ct_alg_ipsec;
	#endif
...
#ifdef CONFIG_IP_NF_ALGCTRL
	#ifdef CONFIG_IP_NF_FTP
	{NET_IPV4_NF_CONNTRACK_ALG_FTP, "ip_ct_alg_ftp",
	 &ip_ct_alg_ftp, sizeof(int), 0644, NULL,
	 &proc_dointvec},
	#endif
	#ifdef CONFIG_IP_NF_H323
	{NET_IPV4_NF_CONNTRACK_ALG_H323, "ip_ct_alg_h323",
	 &ip_ct_alg_h323, sizeof(int), 0644, NULL,
	 &proc_dointvec},
	#endif
	#ifdef CONFIG_IP_NF_AMANDA
	{NET_IPV4_NF_CONNTRACK_ALG_AMANDA, "ip_ct_alg_amanda",
	 &ip_ct_alg_amanda, sizeof(int), 0644, NULL,
	 &proc_dointvec},
	#endif
	#ifdef CONFIG_IP_NF_TFTP
	{NET_IPV4_NF_CONNTRACK_ALG_TFTP, "ip_ct_alg_tftp",
	 &ip_ct_alg_tftp, sizeof(int), 0644, NULL,
	 &proc_dointvec},
	#endif
	#ifdef CONFIG_IP_NF_IRC
	{NET_IPV4_NF_CONNTRACK_ALG_IRC, "ip_ct_alg_irc",
	 &ip_ct_alg_irc, sizeof(int), 0644, NULL,
	 &proc_dointvec},
	#endif
	#ifdef CONFIG_IP_NF_PPTP
	{NET_IPV4_NF_CONNTRACK_ALG_PPTP, "ip_ct_alg_pptp",
	 &ip_ct_alg_pptp, sizeof(int), 0644, NULL,
	 &proc_dointvec},
	#endif
	#ifdef CONFIG_IP_NF_IPSEC
	{NET_IPV4_NF_CONNTRACK_ALG_IPSEC, "ip_ct_alg_ipsec",
	 &ip_ct_alg_ipsec, sizeof(int), 0644, NULL,
	 &proc_dointvec},
	#endif
	#ifdef CONFIG_IP_NF_SIP
	{NET_IPV4_NF_CONNTRACK_ALG_SIP, "ip_ct_alg_sip",
	 &ip_ct_alg_sip, sizeof(int), 0644, NULL,
	 &proc_dointvec},
	#endif
#endif	 
	{0}
...
#ifdef CONFIG_IP_CONNTRACK_LIMIT_CONTROL
#define NET_UDP_CONNTRACK_MAX 2090
#define NET_UDP_CONNTRACK_MAX_NAME "udp_conntrack_max"
#define NET_ICMP_CONNTRACK_MAX 2091
#define NET_ICMP_CONNTRACK_MAX_NAME "icmp_conntrack_max"
#define NET_TCP_CONNTRACK_MAX 2092
#define NET_TCP_CONNTRACK_MAX_NAME "tcp_conntrack_max"
#define NET_MAX_RATE_CONNTRACK 2093
#define NET_MAX_RATE_CONNTRACK_NAME "max_rate_per_ip"
#endif
...
static ctl_table ip_ct_netfilter_table[] = {
	...
	{NET_IP_CONNTRACK_MAX, "ip_conntrack_max",
	 ...
	 &proc_dointvec},
#ifdef CONFIG_IP_CONNTRACK_LIMIT_CONTROL
        { NET_UDP_CONNTRACK_MAX, NET_UDP_CONNTRACK_MAX_NAME,
         &udp_conntrack_max, sizeof(udp_conntrack_max), 0644,  NULL,
         proc_dointvec },
        { NET_ICMP_CONNTRACK_MAX, NET_ICMP_CONNTRACK_MAX_NAME,
         &icmp_conntrack_max, sizeof(icmp_conntrack_max), 0644,  NULL,
         proc_dointvec },
        { NET_MAX_RATE_CONNTRACK, NET_MAX_RATE_CONNTRACK_NAME,
         &max_rate_per_ip, sizeof(max_rate_per_ip), 0644,  NULL,
         proc_dointvec },
#endif
	{0}
};
...
#ifdef CONFIG_IP_CONNTRACK_GARBAGE_NEW
	struct proc_dir_entry* conntrack_proc;
	conntrack_proc=proc_net_create("list_conntrack",0,list_new_conntracks);
	if (!conntrack_proc) goto cleanup_init;
	conntrack_proc->owner = THIS_MODULE;
#endif
...
{
	extern int rtkvoip_clear_conntracks(struct file *file, const char *buffer, 
		unsigned long count, void *data);
	proc = create_proc_entry("ip_conntrack_flush", 0, proc_net);
	if (!proc) goto cleanup_proc2;
	proc->write_proc = rtkvoip_clear_conntracks;
}
#endif
...
#ifdef CONFIG_IP_CONNTRACK_GARBAGE_NEW
	proc_net_remove("list_conntrack");
#endif
#else
 cleanup_proc:
	proc_net_remove("ip_conntrack");
