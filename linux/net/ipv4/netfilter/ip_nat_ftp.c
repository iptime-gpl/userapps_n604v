		     ...
		     enum ip_conntrack_info ctinfo)
{
	...
	DEBUGP("calling ip_nat_mangle_tcp_packet (%s)\n", buffer);
...
#define FAST_NAT_FTP_EFM_PATCH
#ifdef FAST_NAT_FTP_EFM_PATCH
	if (ct_ftp_info->ftptype  == IP_CT_FTP_EPSV)
	{
		// 2011-10-11, ysyoo, EFM
		// Don't check a duplicated expected conntrack if Realtek's Fast NAT is running.
		// Realtek's FAST NAT don't handle a conntrack includes a changed destination port in EPSV mode(Extended Passive mode).
		port = ct_ftp_info->port;
	}
	else if (// local client -> external server
	    ((ct_ftp_info->ftptype != IP_CT_FTP_PASV) || !check_internal_subnet(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.ip) ||
	     // local client -> local server : WAN IP
	     (check_internal_subnet(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.ip) && check_internal_subnet(ct->tuplehash[IP_CT_DIR_REPLY].tuple.src.ip)))
	     &&
	     // external client -> local server
	     ((ct_ftp_info->ftptype != IP_CT_FTP_PORT) || check_internal_subnet(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.ip)))
	{
...
int ftp_alg_update_nat(int ftp_index, unsigned short ftp_port_user, int action)
{
	int i, ret = 0;
	char *tmpname;
	...
	if(action == 1)
	{
		...
		ftp[ftp_index].me = THIS_MODULE;
		ftp[ftp_index].flags = 0;
		ftp[ftp_index].expect = ftp_nat_expected;
		tmpname = &ftp_names[ftp_index][0];
		sprintf(tmpname, "ftp-%d", ftp_port_user);
		ftp[ftp_index].name = tmpname;

		ret = ip_nat_helper_register(&ftp[ftp_index]);
		if (ret) {
			printk("ip_nat_ftp: error registering " "helper for port %d\n", ftp_port_user);
			ip_nat_helper_unregister(&ftp[ftp_index]);
			return ret;
		}
		ports_c++;
	} else if (action == 2) 
