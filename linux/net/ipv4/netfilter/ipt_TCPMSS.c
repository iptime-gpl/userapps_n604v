...
unsigned int
opti_ipt_tcpmss_pmtu_target(struct sk_buff **pskb)
...
#if 0
        if((tcph->rst == 1 )|| (tcph->syn == 0 )) return IPT_CONTINUE;
#endif
		  ...
		  void *userinfo)
{
	...
	struct tcphdr *tcph;
	struct iphdr *iph;
	u_int16_t tcplen, newtotlen, oldval, newmss, mtu; //Bard add mtu from internet patch 20080612
	unsigned int i;
	u_int8_t *opt;
	...
	if(tcpmssinfo->mss == IPT_TCPMSS_CLAMP_PMTU) {
		...
		//Brad disable, since we use the patch from internet for clamp to pmtu below 20080612
		//newmss = (*pskb)->dst->pmtu - sizeof(struct iphdr) - sizeof(struct tcphdr);
		//patch start--------------------
		mtu = (*pskb)->dst->pmtu;
		if (in) {
			if (in->mtu <= (sizeof(struct iphdr) + sizeof(struct tcphdr))) {
				if (net_ratelimit())
					printk(KERN_ERR"ipt_tcpmss_target: invalid interface MTU (%d)\n", in->mtu);
				return NF_DROP; /* or IPT_CONTINUE ?? */
			}
			if (in->mtu < mtu)
				mtu = in->mtu;
