	...
	const u_int16_t proto, const u_int16_t dport)
{
    ...
    DEBUGP("%s: i=%p, proto= %d, dport=%d.\n", __FUNCTION__, i, proto, dport);
    for (count = 0; count < i->ports.rcount; count++) { 
    	DEBUGP("%s: Got one, rproto= %d, rport[0..1]=%d, %d.\n", __FUNCTION__, 
	    	i->rproto, i->ports.rport[count], i->ports.rport[count+1]); 

		if ((rproto == proto) && (i->ports.rport[count*2] <= dport) && (i->ports.rport[count*2+1] >= dport)) {
			return 1;
		}
	}
	return 0;
