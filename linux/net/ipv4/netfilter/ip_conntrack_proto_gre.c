...
#define CONFIG_PPTP_DMZ_WORKAROUND
...
#ifdef CONFIG_PPTP_DMZ_WORKAROUND
	//if(check_internal_subnet(iph->saddr))
		remove_gre_connection(iph);
#endif
...
int remove_gre_connection(struct iphdr *iph)
{
        ...
        if(check_internal_subnet(iph->saddr))
        {
                DEBUGP("SRC:gre_mapip_pkt: %08x -> %08x : %08x\n", htonl(iph->saddr), htonl(iph->daddr), key);
                tuple.src.ip = iph->daddr;
                tuple.dst.ip = 0;
        ...
        {
                DEBUGP("DST:gre_mapip_pkt: %08x -> %08x : %08x\n", htonl(iph->saddr), htonl(iph->daddr), key);
                tuple.src.ip = 0;
                tuple.dst.ip = iph->saddr;
                mask.src.ip = 0;
                mask.dst.ip = 0xffffffff;;
        }

        tuple.src.u.all = 0;
        tuple.src.u.gre.key = 0;
        tuple.dst.u.all = 0;
        tuple.dst.u.gre.key = 0;
        tuple.dst.u.gre.protocol = __constant_htons(GRE_PROTOCOL_PPTP);
        tuple.dst.u.gre.version = GRE_VERSION_PPTP;
        tuple.dst.protonum = IPPROTO_GRE;

        mask.src.u.all = 0;
        mask.src.u.gre.key = 0;
        mask.dst.u.all = 0;
        mask.dst.u.gre.key = 0;
        mask.dst.u.gre.version = 0xff;
        mask.dst.u.gre.protocol = 0xffff;
        mask.dst.protonum = 0xffff;

        for (i=0, found = 0; i < ip_conntrack_htable_size; i++)
        {
                tuple_hash = LIST_FIND(
                                  ...
                                  &tuple, &mask);
                if(  tuple_hash  && !(test_bit(IPS_ASSURED_BIT, &tuple_hash->ctrack->status)) &&
                     (tuple_hash == &tuple_hash->ctrack->tuplehash[IP_CT_DIR_ORIGINAL])
                     )
                {
                        found = 1;
                        break;
		...
		if( tuple_hash) 
		{
                        DEBUGP("=> Tuple Found but not ORIGINAL [Status : %08x (%d)]: \n",
                                tuple_hash->ctrack->status,
                                test_bit(IPS_ASSURED_BIT, &tuple_hash->ctrack->status));
			DUMP_TUPLE_GRE(&tuple_hash->ctrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple);
		}
        }

        if( found )
        {
                DEBUGP("=> Found :Tuple hash [Status : %08x (%d)]: \n",
                        tuple_hash->ctrack->status,
                        test_bit(IPS_ASSURED_BIT, &tuple_hash->ctrack->status));
		DUMP_TUPLE_GRE(&tuple_hash->ctrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple);
