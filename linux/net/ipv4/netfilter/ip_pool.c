...
MODULE_LICENSE("GPL");
#define ALLIP_FLAG
#define CONFIG_IP_POOL_STATISTICS
...
static int pool_change(ip_pool_t index, u_int32_t addr, int isdel)
{
	...
	pool = lookup(index);
	if(pool->type != BITMAP_TYPE ) return -1;
...
static int getpool(
	...
	case IP_POOL_FLUSH:
		...
		if(pool)
		{
			...
			if(pool->type == BITMAP_TYPE && pool->members) 
				memset( pool->members, 0x0, pool->member_size );
	...
	case IP_POOL_DESTROY:
		...
		if(pool)
		{
			...
			pool->nr_use = 0x0;
			pool->nr_match = 0x0;
			if(pool->type == BITMAP_TYPE ) 
			{
				if(pool->members) kfree(pool->members);
				pool->members = NULL;
			}

			write_unlock_bh(&pool->lock);
			DP("ip_pool DESTROY %d\n", req.index);
		}
		else
			return -EINVAL;
		...
		return 0;
	case IP_POOL_INIT:
		DP("ip_pool INIT 0x%08x-0x%08x %x\n", req.addr, req.addr2, req.type);
		pool = lookup(req.index);
		if (!pool)
			return -EINVAL;
		if(pool->type != 0x0 )
		{
			DP("IPPOOL idx=%d Already Exists..\n",req.index );
			return -EINVAL;
		}
		...
		if ((req.type == BITMAP_TYPE) && pool->members) {
