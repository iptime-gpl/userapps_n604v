		      ...
		      enum ip_conntrack_info conntrackinfo)
{
#ifdef CONFIG_NETFILTER_SORIBADA_PATCH
        /* ysyoo, 2002.11.11 for soribada */
        if ( (conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.udp.port == htons(SORIBADA_UDP_PORT) &&
              conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u.udp.port == htons(SORIBADA_UDP_PORT)) ||

             (conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.udp.port == htons(SORIBADA_SEARCH_PORT) &&
              conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u.udp.port == htons(SORIBADA_SEARCH_PORT))
           )
        {
                ip_ct_refresh(conntrack, UDP_TIMEOUT_1SEC*2);
        }
        else if (conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.udp.port == htons(REALP2P_UDP_PORT))
        {
                ip_ct_refresh(conntrack, UDP_TIMEOUT_1SEC*2);
        }
        else if (conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.udp.port == htons(67))
        {
                ip_ct_refresh(conntrack, UDP_TIMEOUT_1SEC);
        }
        else if (this_udp_is_app_timeout(conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.udp.port) ||
                 this_udp_is_app_timeout(conntrack->tuplehash[IP_CT_DIR_REPLY].tuple.dst.u.udp.port))
        {
                ip_ct_refresh(conntrack, UDP_TIMEOUT_1SEC * 2);
        }
        /* Samsung Wzy070 request, udp_timeout is 600 */
        else if ( (conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.udp.port == htons(SIP_UDP_PORT)) ||
                (conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.udp.port == htons(WYZ070_SMS_UDP_PORT)) )
        {
	...
	if (test_bit(IPS_SEEN_REPLY_BIT, &conntrack->status)) {
                ...
                if (conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.udp.port == htons(UDP_PORT_ISAKMP))
