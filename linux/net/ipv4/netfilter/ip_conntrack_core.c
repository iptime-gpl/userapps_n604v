...
int tcp_conntrack_max = 0;
int udp_conntrack_max = CONFIG_UDP_CONN_MAX_SIZE;
int icmp_conntrack_max = CONFIG_ICMP_CONN_MAX_SIZE;
#endif
...
#define MAX_RATE_IP_NUM 258  /* 0: local, 1 ~ 254 : ipidx, 255:unknown, 256:WAN1 twin ip, 257:WAN2 twin_ip */
...
void print_tuple_ip( char *msg, struct ip_conntrack_tuple *org_tuple, struct ip_conntrack_tuple *repl_tuple )
{
        printk("%s --> ", msg);
        if(org_tuple)
        {
                printk("SRC:%d.%d.%d.%d ",
                                (htonl(org_tuple->src.ip)>>24)&0xff,
                                (htonl(org_tuple->src.ip)>>16)&0xff,
                                (htonl(org_tuple->src.ip)>>8)&0xff,
                                htonl(org_tuple->src.ip)&0xff);
                printk("DST:%d.%d.%d.%d ",
                                (htonl(org_tuple->dst.ip)>>24)&0xff,
                                (htonl(org_tuple->dst.ip)>>16)&0xff,
                                (htonl(org_tuple->dst.ip)>>8)&0xff,
                                htonl(org_tuple->dst.ip)&0xff);
        }

        if(repl_tuple)
        {
                printk("SRC:%d.%d.%d.%d ",
                                (htonl(repl_tuple->src.ip)>>24)&0xff,
                                (htonl(repl_tuple->src.ip)>>16)&0xff,
                                (htonl(repl_tuple->src.ip)>>8)&0xff,
                                htonl(repl_tuple->src.ip)&0xff);
                printk("DST:%d.%d.%d.%d ",
                                (htonl(repl_tuple->dst.ip)>>24)&0xff,
                                (htonl(repl_tuple->dst.ip)>>16)&0xff,
                                (htonl(repl_tuple->dst.ip)>>8)&0xff,
                                htonl(repl_tuple->dst.ip)&0xff);
        }
        printk("\n");
}
#endif
...
static int get_conntrack_count_per_ip_index( struct ip_conntrack_tuple *org_tuple, struct ip_conntrack_tuple *repl_tuple )
{
        int ipidx = 0;

        if(org_tuple && check_local_ip(org_tuple->dst.ip))
                ipidx = LANCPU_IPIDX;
        else if(org_tuple && check_internal_subnet(org_tuple->src.ip))
                ipidx = get_ipidx_by_ipaddr(org_tuple->src.ip);
        else if(repl_tuple && check_internal_subnet(repl_tuple->src.ip))
                ipidx = get_ipidx_by_ipaddr(repl_tuple->src.ip);
        else
                ipidx = WANCPU_IPIDX; /* connection from wan */
        return ipidx;
}

static int check_conntrack_ratio_per_ip(struct ip_conntrack_tuple *org_tuple, int *passme )
{
        int ipidx;
        ...
        *passme = 0;
        ...
        if(!ip_conntrack_max || !max_rate_per_ip )
                return 1;
        if(conntrack_count_per_ip[ipidx]*100/ip_conntrack_max < max_rate_per_ip )
                return 1;
        return 0;
}
...
int check_conntrack_ratio_in_dnat(struct ip_conntrack *ct, const struct ip_nat_multi_range *mr)
{
	...
	if(check_internal_subnet(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.ip))
        ...
        if( !ip_conntrack_max || !max_rate_per_ip ||
             conntrack_count_per_ip[ipidx]*100/ip_conntrack_max < max_rate_per_ip )
        {
#ifdef DEBUG_CONNTRACK_LIMIT
...
#if 0
		if( h )
			printk("loose UDP packet match : [%lx / %x] -> [%lx / %x]\n",
		...
		if(drop_priority[i].state < TCP_CONNTRACK_MAX )	 //ramen--drop tcp conntrack	
		{
			...
			{		
				...
				list_for_each(it, ptr) {
					...
					search_count++;
					if(((dump_tuple_test->dst.u.tcp.port==MSN_LOGIN) || (dump_tuple_test->dst.u.tcp.port == MSN_CONTROL)) && ((ip_conntrack_max-atomic_read(&ip_conntrack_count)) > threshold) ){
						continue;
					}else if((dump_tuple_test->dst.u.tcp.port ==HTTP_ACCESS) &&(!(test_bit(IPS_BT_SESSION_BIT, &tmp->status))) && ((ip_conntrack_max-atomic_read(&ip_conntrack_count)) > threshold)){
						continue;
					}else if((dump_tuple_test->dst.u.tcp.port ==HTTP_ACCESS) &&((test_bit(IPS_BT_SESSION_BIT, &tmp->status)))){
						isDropped = 0;
					...
					}else if(((dump_tuple_test->dst.u.tcp.port==UPNP_IGD) || (dump_tuple_test->dst.u.tcp.port == UPNP_WSC)) && (dump_tuple_test->dst.ip == lan_ipaddress)  && ((ip_conntrack_max-atomic_read(&ip_conntrack_count)) > threshold)){
						continue;
					}else{
		...
		else //ramen--drop udp conntrack
 		{
	 		...
	 		if(!list_empty(ptr))
			{
				...
				list_for_each(it, ptr) 
				{
					...
					dump_tuple_test = &(orig_tuple.tuple);	
					search_count++;
					if((dump_tuple_test->dst.u.udp.port ==53) && ((ip_conntrack_max-atomic_read(&ip_conntrack_count)) > threshold)){
						continue;
					}else{
						//if (((((tmp->timeout.expires - jiffies)/HZ) >> factor)<=drop_priority[i].threshold ) && del_timer(&tmp->timeout)) 
						isDropped = 0;
	       ...
	       struct sk_buff *skb)
{
...
#ifdef CONFIG_IP_CONNTRACK_LIMIT_CONTROL
        if( icmp_conntrack_max && (tuple->dst.protonum == IPPROTO_ICMP ) && (icmp_conn_count.counter >= icmp_conntrack_max) )
        {
#ifdef CONFIG_IP_NF_TARGET_NETDETECT
                update_net_detect_history(tuple->src.ip, IPPROTO_ICMP, 0);
#endif
                return ERR_PTR(-ENOMEM);
        }
        else if( udp_conntrack_max && (tuple->dst.protonum == IPPROTO_UDP ) && (udp_conn_count.counter >= udp_conntrack_max) )
                return ERR_PTR(-ENOMEM);
        else if( tcp_conntrack_max && (tuple->dst.protonum == IPPROTO_TCP ) && (tcp_conn_count.counter >= tcp_conntrack_max) )
                return ERR_PTR(-ENOMEM);
#endif
...
#ifdef CONFIG_IP_CONNTRACK_LIMIT_PER_IP
        if(!check_conntrack_ratio_per_ip((struct ip_conntrack_tuple *)tuple, &passme))
                return ERR_PTR(-ENOMEM);
#endif
	
#ifdef CONFIG_IP_CONNTRACK_LIMIT_PER_IP

	if (!passme && ip_conntrack_max && atomic_read(&ip_conntrack_count) >= ip_conntrack_max) {
	...
	conntrack->tuplehash[IP_CT_DIR_REPLY].ctrack = conntrack;
        ...
        conntrack->tuplehash[IP_CT_DIR_ORIGINAL].packets = 0;
        conntrack->tuplehash[IP_CT_DIR_ORIGINAL].bytes = 0;
        conntrack->tuplehash[IP_CT_DIR_REPLY].packets = 0;
        conntrack->tuplehash[IP_CT_DIR_REPLY].bytes = 0;
#endif
	...
	if (expected) {
        ...
        else if(tuple->dst.protonum == IPPROTO_ICMP )
...
#ifdef CONFIG_IP_CONNTRACK_LIMIT_PER_IP
	increase_conntrack_count_per_ip(conntrack);
#endif
...
#ifdef CONFIG_CONNTRACK_STATISTICS
        h->ctrack->tuplehash[DIRECTION(h)].packets++;
        h->ctrack->tuplehash[DIRECTION(h)].bytes += ntohs(skb->nh.iph->tot_len);
#endif
...
#ifdef CONFIG_ICMP_FRAGMENTATION_DROP
			     ...
			     const struct ip_conntrack_tuple *newreply)
{
	...
	if (
            ...
            !conntrack->port_restricted_cone &&
#endif
		__ip_conntrack_find(newreply, conntrack)
		) {
		WRITE_UNLOCK(&ip_conntrack_lock);
		return 0;
	}
...
#ifdef CONFIG_IP_NF_LOOSEUDP // ysyoo
extern int sysctl_ip_masq_udp_dloose;
#endif
int
ip_ct_port_restricted_cone_nat_tuple(struct ip_conntrack_tuple *tuple, struct ip_conntrack *conntrack)
{
        struct ip_conntrack_tuple target_tuple, target_mask;
        struct ip_conntrack_tuple_hash *tuple_hash = NULL;
        int i;

        target_tuple.src.ip = conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.ip;
        target_tuple.src.u.all = conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u.all;
        target_tuple.dst.ip = 0;
        target_tuple.dst.u.all = conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.all;
        target_tuple.dst.protonum = conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.protonum;
        target_mask.src.ip = 0xffffffff;
        target_mask.src.u.udp.port = 0xffff;
        target_mask.dst.ip = 0;
#ifdef CONFIG_IP_NF_LOOSEUDP // ysyoo
	if(sysctl_ip_masq_udp_dloose)
	{
		target_mask.dst.u.udp.port = 0;
		target_mask.dst.protonum = 0;
	} else
#endif
	{
		target_mask.dst.u.udp.port = 0xffff;
		target_mask.dst.protonum = 0xffff;
	}
		...
		{
                printk("=> ORG tuple %u %u.%u.%u.%u:%hu -> %u.%u.%u.%u:%hu\n",  \
                        conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.protonum,
                        NIPQUAD(conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.ip),
                        ntohs(conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u.udp.port),
                        NIPQUAD(conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.ip),
                        ntohs(conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.udp.port));
                printk("=> PORT Restricted Cone Old tuple %p: %u %u.%u.%u.%u:%hu -> %u.%u.%u.%u:%hu\n",       \
                       tuple, tuple->dst.protonum,                              \
...
#endif
		if (tuple->dst.protonum == IPPROTO_TCP)
			tuple->src.u.tcp.port = tuple_hash->ctrack->tuplehash[IP_CT_DIR_REPLY].tuple.dst.u.tcp.port;
		else if (tuple->dst.protonum == IPPROTO_UDP)
			tuple->src.u.udp.port = tuple_hash->ctrack->tuplehash[IP_CT_DIR_REPLY].tuple.dst.u.udp.port;
		else
			tuple->src.u.all = tuple_hash->ctrack->tuplehash[IP_CT_DIR_REPLY].tuple.dst.u.all;

                conntrack->port_restricted_cone = 1;

                invert_tuplepr(&inv_tuple, tuple);
                h = ip_conntrack_find_get(&inv_tuple, NULL);
                if(h && del_timer( &h->ctrack->timeout ))
                                h->ctrack->timeout.function( (unsigned long)h->ctrack );

#if 1
		if (tuple->dst.protonum == IPPROTO_UDP)
		{
                printk("=> PORT Restricted Cone New tuple %p: %u %u.%u.%u.%u:%hu -> %u.%u.%u.%u:%hu\n", \
                       tuple, tuple->dst.protonum,                              \
                       NIPQUAD(tuple->src.ip), ntohs(tuple->src.u.udp.port),         \
                       NIPQUAD(tuple->dst.ip), ntohs(tuple->dst.u.udp.port));
		}
...
#ifndef CONFIG_IP_CONNTRACK_MAX_DEFINE
static int hashsize = 0;
#else
static int hashsize = (CONFIG_IP_CONNTRACK_MAX_SIZE / 8) ;
#endif
...
static int proc_read_conntrack_info(char *buffer, char **start, off_t offset, int length, int *eof, void *data)
{
        ...
        p +=  sprintf(p, "total:%d(%d), tcp:%d(%d), udp:%d(%d), icmp:%d(%d)",
                        ip_conntrack_count.counter,ip_conntrack_max,
...
#ifndef CONFIG_IP_CONNTRACK_MAX_DEFINE
        ip_conntrack_max = 8 * ip_conntrack_htable_size;
        if (ip_conntrack_max < 2048)
                ip_conntrack_max = 2048;
#else
        ip_conntrack_max = CONFIG_IP_CONNTRACK_MAX_SIZE;
#endif
	...
	init_timer(&tcp_patten_ck);
      ...
      mod_timer(&tcp_patten_ck, jiffies + 100);
...
#ifdef CONFIG_IP_CONNTRACK_LIMIT_CONTROL
        {
                ...
                proc = create_proc_entry("net/conntrack_info",0,0);
                if(proc)
...
#ifdef CONFIG_IP_CONNTRACK_LIMIT_PER_IP
        {
                ...
                proc = create_proc_entry("net/conntrack_ip_stat",0,0);
                if(proc)
