 ...
 *
 *	Version: $Id: ipmr.c,v 1.4 2012/09/05 09:27:04 rtlwl Exp $
 *
 *	Fixes:
...
#define NUM2PRINT 50
	...
	printk("sar: len = %d skb: %s\n", skb->len, buf);
}
#else
#define skb_debug(skb)	do {} while (0)
#endif

// 2004, Casey, for IGMPPROXY
...
int ipmr_mfc_delete(struct mfcctl *mfc)
{
	...
	for (cp=&mfc_cache_array[line]; (c=*cp) != NULL; cp = &c->next) {
		...
		printk ("src : %08x:%08x --- mgroup : %08x %08x \n", c->mfc_origin, mfc->mfcc_origin.s_addr,
		...
		if (c->mfc_origin == mfc->mfcc_origin.s_addr &&
			...
			*cp = c->next;
			...
			printk("====> DEL OK - %08x:%08x\n", c->mfc_origin, c->mfc_mcastgrp);
			kmem_cache_free(mrt_cachep, c);
			return 0;
		}
	}
...
int ipmr_mfc_add(struct mfcctl *mfc, int mrtsock)
{
	...
	// Kaohj -- keep the original ip
	origin = mfc->mfcc_origin.s_addr;
	...
	c->mfc_origin=mfc->mfcc_origin.s_addr;
	c->mfc_mcastgrp=mfc->mfcc_mcastgrp.s_addr;
	// Kaohj --- keep the origin IP
	c->mfc_firstone = origin;
	c->mfc_parent=mfc->mfcc_parent;
	ipmr_update_threshoulds(c, mfc->mfcc_ttls);
	if (!mrtsock)
		c->mfc_flags |= MFC_STATIC;
...
int ip_mroute_setsockopt(struct sock *sk,int optname,char *optval,int optlen)
{
	...
	switch(optname)
	{
		...
		case MRT_DEL_MFC:
			...
			{
				printk("----> MRT_DEL_MFC %08x:%08x\n", mfc.mfcc_origin.s_addr,mfc.mfcc_mcastgrp.s_addr);
				ret = ipmr_mfc_delete(&mfc);
			}
			else
			{
				printk("----> MRT_ADD_MFC %08x:%08x\n", mfc.mfcc_origin.s_addr,mfc.mfcc_mcastgrp.s_addr);
