 ...
 *
 *	Version: $Id: igmp.c,v 1.1.1.1 2010/06/15 01:48:41 ysyoo Exp $
 *
 *	Authors:
...
// Kaohj --- for Telefonica special case
//#define	ORIGIN_IP_PASSTHROUGH
...
// Kaohj
#ifdef ORIGIN_IP_PASSTHROUGH
extern struct mfc_cache *ipmr_cache_find(__u32 origin, __u32 mcastgrp);
#endif
#ifdef CONFIG_RTL_KERNEL_MIPS16_IPV4
__NOMIPS16
#endif
static int igmp_send_report(struct net_device *dev, u32 group, int type)
{
	...
	iph->ttl      = 1;
	iph->daddr    = dst;
	// Kaohj --- let origin IP to be passthrough
#ifdef ORIGIN_IP_PASSTHROUGH
	u = ipmr_cache_find(0, group);
	if (u) {
		//printk("grp=%x, src=%x\n", u->mfc_mcastgrp, u->mfc_firstone);
		iph->saddr = u->mfc_firstone;
	}
	else
		iph->saddr    = rt->rt_src;
#else
...
int igmp_rcv(struct sk_buff *skb)
{
	...
	default:
		break;
		//printk(KERN_DEBUG "New IGMP type=%d, why we do not know about it?\n", ih->type); //Brad comment out
	}
	in_dev_put(in_dev);
	kfree_skb(skb);
	return 0;
...
int ip_check_mc(struct in_device *in_dev, u32 mc_addr)
{
	...
	// 20040517, Casey for IGMP proxy, receive all IGMP packet
	// for IGMP packets that are not local multicast
