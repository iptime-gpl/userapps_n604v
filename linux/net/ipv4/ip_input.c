 ...
 *
 * Version:	$Id: ip_input.c,v 1.4 2012/04/01 22:11:41 rtlwl Exp $
 *
...
static inline int ip_local_deliver_finish(struct sk_buff *skb)
{
...

	{
		...
		if(raw_sk != NULL) {	/* Shift to last raw user */
			...
			if (skb->nh.iph->protocol != IPPROTO_GRE)
#endif
...
static inline int ip_rcv_finish(struct sk_buff *skb)
{
	...
	if (skb->dst == NULL) {
		...
		if (ip_route_input(skb, iph->daddr, iph->saddr, iph->tos, dev))
			goto drop; 
#ifdef JACKSON_NET_TRACE
                printk("Y: saddr=%08x, skb=%08x", iph->saddr, (unsigned int)skb);
#endif
...
#ifdef JACKSON_NET_TRACE
	//printk("2.At %08X: skb->dst->input=%08x\n", jiffies, (unsigned int)skb->dst->input);
#endif	
...
int sysctl_nat_passthrough_flag;
#endif
