 ...
 *
 * Version:	$Id: fib_hash.c,v 1.1.1.1 2010/06/15 01:48:41 ysyoo Exp $
 *
...
const char *tbName[] = { "UNSPEC", "DEFAULT", "MAIN", "LOCAL" };
const char *tpName[] = { "UNSPEC", "UNICAST", "LOCAL", "BRAODCAST" };
#define FTprint(a...)
/*
   printk(a)
*/
...
FTprint("\n\ntb(%s)_insert: %s %08x/%d %d %08x\n\n", tb->tb_id ? tbName[tb->tb_id-252] : tbName[0], tpName[r->rtm_type], rta->rta_dst ?
...
rta->rta_prefsrc ? *(u32*)rta->rta_prefsrc : 0);
	...
	if (f && 
		...
		if (n->nlmsg_flags&NLM_F_REPLACE) {
			...
			{
				FastPath_hook2(rta->rta_dst ? *(u32*)rta->rta_dst : 0, inet_make_mask(z),rta->rta_gw ? *(u32*)rta->rta_gw : 0, (uint8 *)fi->fib_dev->name, RT_NONE);
				
			}
		#else
			rtk_modifyRoute(rta->rta_dst ? *(u32*)rta->rta_dst : 0, inet_make_mask(z),rta->rta_gw ? *(u32*)rta->rta_gw : 0, (uint8 *)fi->fib_dev->name, RT_NONE);
...
create:
			...
			{
				FastPath_hook3(rta->rta_dst ? *(u32*)rta->rta_dst : 0, inet_make_mask(z),rta->rta_gw ? *(u32*)rta->rta_gw : 0, (uint8 *)fi->fib_dev->name, RT_NONE);
			}
		#else
	rtk_addRoute(rta->rta_dst ? *(u32*)rta->rta_dst : 0, inet_make_mask(z),rta->rta_gw ? *(u32*)rta->rta_gw : 0, (uint8 *)fi->fib_dev->name, RT_NONE);
	...
	if (del_fp) {
		...
		*del_fp = f->fn_next;
		...
		if (tb->tb_id == RT_TABLE_MAIN) {
			...
			ipDst = (rta->rta_dst) ? *(u32*)rta->rta_dst : 0;
			ipMask = inet_make_mask(z);
			ipGw = (rta->rta_gw) ? *(u32*)rta->rta_gw : 0;
//			if (!ipDst || (!MULTICAST(ipDst) && !LOOPBACK(ipDst))) { 
			if (!ipDst || (!MULTICAST(ipDst) && !LOOPBACK(ipDst) && (ipDst != 0xffffffff))) { 
				...
				LR_DEBUG("LR(delRoute_Replace): dst:%u.%u.%u.%u/%d gw:%u.%u.%u.%u, errno = %d, ifidx = %d\n", 
                        NIPQUAD(ipDst), z, NIPQUAD(ipGw), rc, *rta->rta_oif);
			}
		}
#endif
...
//			if (!ipDst || (!MULTICAST(ipDst) && !LOOPBACK(ipDst)) ) { 
			if (!ipDst || (!MULTICAST(ipDst) && !LOOPBACK(ipDst) && (ipDst != 0xffffffff))) { 
				rc = rtl865x_addRoute(ipDst, ipMask, ipGw, fi->fib_dev->name, RT_NONE);
				LR_DEBUG("LR(addRoute): dst:%u.%u.%u.%u/%d gw:%u.%u.%u.%u, errno = %d, ifidx = %d\n", 
						NIPQUAD(ipDst), z, NIPQUAD(ipGw), rc, *rta->rta_oif);
			}
		}
#endif
...
#if 0 // #ifdef CONFIG_RTL865X_HW_TABLES
	if (tb->tb_id == RT_TABLE_MAIN && gHwNatEnabled == 1) {
		struct net_device *netif=NULL;
		unsigned int dst2, mask, gw;
		unsigned int dst3, mask3, isWan;
		char *dev_t;
		int rc;

		if (rta->rta_oif)
			netif = dev_get_by_index(*rta->rta_oif);
		dst2		= (rta->rta_dst)? *(u32*)rta->rta_dst: 0;
		mask	= inet_make_mask(z);
		gw		= (rta->rta_gw)? *(u32*)rta->rta_gw: 0;
		dev_t	= (netif)? netif->name: NULL; 

		if (dst2 == 0xe0000000 || dst2 == 0xef000000 || dst2 == 0xffffffff)
			return 0;
		
		if (dst2 != 0 && (rta->rta_oif)) {
			isWan = rtl865x_netif_isWan(dev_t);
			if (isWan) {
				dst3 = wdst;
				mask3 = wmask;
			}
			...
			rc = rtl865x_delRoute(dst3, mask3);
			LR_DEBUG("LR(del_rt2): dst:%u.%u.%u.%u/0x%x,  errno=%d, ifidx= %d\n", 
                        NIPQUAD(dst3), mask3, rc, *rta->rta_oif);
		}

		rc = rtl865x_addRoute(dst2, mask, gw, dev_t, 0);
		if (rc == 0 && dst2 != 0 && (rta->rta_oif)) {
			if (isWan) {
				wdst = dst2;
				wmask = mask;
			}
		...
		if (rc == 0 && netif)
			rtl865x_updateNetIfTabMac(netif->name, netif->dev_addr);
		LR_DEBUG("LR(%s): dst:%u.%u.%u.%u/%u, gw:%u.%u.%u.%u, dev: %s, errno=%d, ifidx= %d\n", 
				"add_rt", NIPQUAD(dst2), z, NIPQUAD(gw), dev_t? dev_t: "null", rc, *rta->rta_oif
		);
	}
#endif
...
FTprint("\n\ntb(%s)_delete: %s %08x/%d %d\n", tb->tb_id ? tbName[tb->tb_id-252] : tbName[0], tpName[r->rtm_type], rta->rta_dst ?
	...
	if (del_fp) {
		...
		FastPath_hook1(rta->rta_dst ? *(u32*)rta->rta_dst : 0, inet_make_mask(z));
		#else
		rtk_delRoute(rta->rta_dst ? *(u32*)rta->rta_dst : 0, inet_make_mask(z));
                #endif
#endif
#ifdef CONFIG_RTL865X_HW_TABLES
		if (tb->tb_id == RT_TABLE_MAIN) {
...
//			if (!ipDst || (!MULTICAST(ipDst) && !LOOPBACK(ipDst))) { 
			if (!ipDst || (!MULTICAST(ipDst) && !LOOPBACK(ipDst) && (ipDst != 0xffffffff))) { 
				rc = rtl865x_delRoute(ipDst, ipMask);
				LR_DEBUG("LR(delRoute): dst:%u.%u.%u.%u/%d gw:%u.%u.%u.%u, errno = %d, ifidx = %d\n", 
                        NIPQUAD(ipDst), z, NIPQUAD(ipGw), rc, *rta->rta_oif);
			}
		}
#endif
...
#ifdef CONFIG_RTL865X_HW_TABLES
			if (mainRT) {
				fn_key_t key = f->fn_key;
				int rc;
				unsigned int ipDst, ipMask, ipGw;

				ipDst = key.datum;
				ipMask = inet_make_mask(z);
				ipGw = fi ? fi->fib_nh[0].nh_gw : 0;
//				if (!ipDst || (!MULTICAST(ipDst) && !LOOPBACK(ipDst))) { 
				if (!ipDst || (!MULTICAST(ipDst) && !LOOPBACK(ipDst) && (ipDst != 0xffffffff))) { 
					rc = rtl865x_delRoute(ipDst, ipMask);
					LR_DEBUG("LR(delRoute): dst:%u.%u.%u.%u/%d gw:%u.%u.%u.%u, errno = %d, ifidx = %s\n", 
							NIPQUAD(ipDst), z, NIPQUAD(ipGw), rc, fi ? fi->fib_dev->name : "NULL");
				}
			}
#endif
...
static int fn_hash_flush(struct fib_table *tb)
{
	...
	int found = 0;
#ifdef CONFIG_RTL865X_HW_TABLES
	u32 mainRT = (tb->tb_id == RT_TABLE_MAIN) ? 0x80000000 : 0;
#endif	
...
#ifdef CONFIG_RTL865X_HW_TABLES	
			tmp += fn_flush_list(&fz->fz_hash[i], (fz->fz_order|mainRT), table);
#else
