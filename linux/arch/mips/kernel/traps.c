...
extern asmlinkage void IA7546RIexception(void);
...
void show_trace(unsigned int *sp)
{
	...
	printk("\n  uptime=%lu sec. ", jiffies / 100);
#ifdef CONFIG_RTK_STACK_CHECK
	printk("\n  total stack size: %d bytes, remained minimum stack size ever: %d bytes. ", 
		KERNEL_STACK_SIZE, rtk_stack_check());
#endif
	printk("\nCall Trace:");
...
asmlinkage void do_cpu(struct pt_regs *regs)
{
	...
	int sig;
#if 0
	printk("do_cpu: badaddr=%08x, epc=%08x,cp0_status=%X,cp0_cause=%X\n", 
			regs->cp0_badvaddr,regs->cp0_epc,regs->cp0_status,regs->cp0_cause);
#endif
...
fp_emul:
	...
	return;
#else
	printk("Error: Kernel Floating Point Emulator is not supported\n");
#endif
	//printk("Caught FP inst, EPC=%08x\n", regs->cp0_epc);
	compute_return_epc(regs);
	return;
