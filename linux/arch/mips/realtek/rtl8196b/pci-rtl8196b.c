...
void __init pcibios_fixup_irqs(void)
{
	...
	pci_for_each_dev(dev) {
		if (dev->bus->number < pci1_bus_number) {
				...
				if(is8196C)
				{
					dev->irq = PCIE_IRQ_96C;
					rtlpci0_write_config_byte(dev, PCI_INTERRUPT_LINE, PCIE_IRQ_96C);					
				}
				else 
				{
					dev->irq = PCIE_IRQ_96E;
					rtlpci0_write_config_byte(dev, PCI_INTERRUPT_LINE, PCIE_IRQ_96E);
				}
#else		
				dev->irq = PCIE_IRQ;
