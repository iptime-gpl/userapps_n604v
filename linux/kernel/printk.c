...
/*
 * Modified for
 * 1. separating log buffer and priting buffer
 * 2. adding scrlog_printk() that just log message but not print out
 * 2003/09/18 by Jimmy Lin <jimmylin@realtek.com.tw>
 */
...
/*
 * separate log buffer and printing buffer
 */
static char prt_buf[LOG_BUF_LEN];
...
static unsigned long prt_end;			/* Index into prt_puf: most-recently-written-char + 1 */
...
static void emit_log_char(char c, int prt_to_con)
{
	...
	if (prt_to_con) {
		PRT_BUF(prt_end) = c;
		prt_end++;
		if (prt_end - prt_start > LOG_BUF_LEN)
			prt_start = prt_end - LOG_BUF_LEN;
	}
}
	...
	for (p = printk_buf; *p; p++) {
		if (log_level_unknown) {
			if (p[0] != '<' || p[1] < '0' || p[1] > '7' || p[2] != '>') {
				emit_log_char('<', 1);
				emit_log_char(default_message_loglevel + '0', 1);
				emit_log_char('>', 1);
			}
			log_level_unknown = 0;
		}
...
asmlinkage int scrlog_printk(const char *fmt, ...)
{
	...
	for (p = printk_buf; *p; p++) {
		if (log_level_unknown) {
			if (p[0] != '<' || p[1] < '0' || p[1] > '7' || p[2] != '>') {
				emit_log_char('<', 0);
				emit_log_char(default_message_loglevel + '0', 0);
				emit_log_char('>', 0);
			}
			log_level_unknown = 0;
		}
